"""Historical single-thread-BLAS binary required. Fixed Si Taylor+ACE MPI/OpenMP comparison; macOS RSS sampling, sequential MPI runs."""
...
def main():
 ...
 out=root/'docs/results/si-hse-hybrid';out.mkdir(exist_ok=True)
 ...
 try:
  for rep in range(2):
   for ranks,threads in ([(16,1),(8,2),(4,4),(8,1),(4,1)] if rep==0 else [(4,1),(8,1),(4,4),(8,2),(16,1)]):
    name=f'hybrid_taylor_n{ranks}_t{threads}_r{rep}';d=base/name;d.mkdir(exist_ok=True)
    ...
    with (d/'inputfile').open() as inp,(d/'run.log').open('w') as log:
     proc=subprocess.Popen(['mpiexec','--bind-to','none','-n',str(ranks),'/usr/bin/env',f'OMP_NUM_THREADS={threads}','OMP_DYNAMIC=FALSE','OMP_PROC_BIND=FALSE','OPENBLAS_NUM_THREADS=1',exe],cwd=d,stdin=inp,stdout=log,stderr=subprocess.STDOUT)
    ...
    ref=base/f'hybrid_taylor_n16_t1_r0/checkpoint_rt_{args.steps:06d}/wfn.bin'
