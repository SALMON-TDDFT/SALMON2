...
def run(export,state,output,steps=1,dt=.32,amplitude=1e-4):
 ...
 with Semilocal('hse06') as xc:
  ...
  try:
   ...
   pt_norm=float(np.linalg.norm(pt_residual(u,initial_action,m.dv)));sch_norm=float(np.linalg.norm(initial_action))
 ...
 result=dict(completed=True,steps=steps,dt_au=dt,amplitude=amplitude,method='PT-CN-ACE, Jia/Lin Eq.12',
  ...
  initial_pt_to_schrodinger_derivative_norm=pt_norm/sch_norm,energy_change_Ha=final_energy-initial_energy,
  ...
  wall_seconds=time.perf_counter()-start,blas_threads=os.environ.get('OPENBLAS_NUM_THREADS'),fftw_threads=1,
