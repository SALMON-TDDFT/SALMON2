...
grep -Fq "Alternate implementation checks [GPa/Hartree]" "$write_f90"
grep -Fq "Tr(har_shadow)*V + E_h" "$write_f90"
grep -Fq "P_har_shadow [GPa]" "$write_f90"
grep -Fq "P_har_shadow - P_har [GPa]" "$write_f90"

! rg -F -q "P_har_shadow+loc_fullobj+ewald [GPa]" "$write_f90"
