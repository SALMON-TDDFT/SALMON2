...
grep -Fq 'call reset_xc_operator_payload(payload)' "$repo_root/src/xc/salmon_xc.f90"
grep -Fq 'call reset_xc_operator_payload(system%xc_payload)' "$repo_root/src/xc/salmon_xc.f90"
grep -Fq 'payload%vtau_has_shadow_values = .false.' "$repo_root/src/xc/salmon_xc.f90"
grep -Fq 'system%xc_payload' "$repo_root/src/xc/salmon_xc.f90"
...
grep -Fq 'subroutine fail_tau_operator(message)' "$repo_root/src/common/hamiltonian.f90"
grep -Fq 'Error in tau operator: ' "$repo_root/src/common/hamiltonian.f90"
grep -Fq 'call fail_tau_operator("support is unavailable for OpenACC builds")' "$repo_root/src/common/hamiltonian.f90"
...
if grep -Fq 'type(s_xc_operator_payload) :: xc_payload' "$repo_root/src/gs/scf_iteration.f90"; then
  exit 1
fi
if grep -Fq 'type(s_xc_operator_payload) :: xc_payload' "$repo_root/src/gs/initialization_dft.f90"; then
  exit 1
fi
if grep -Fq 'copy_xc_operator_payload' "$repo_root/src/gs/scf_iteration.f90"; then
  exit 1
fi
if grep -Fq 'copy_xc_operator_payload' "$repo_root/src/gs/initialization_dft.f90"; then
  exit 1
fi
if grep -Fq 'xc_payload=' "$repo_root/src/gs/scf_iteration.f90"; then
  exit 1
fi
if grep -Fq 'xc_payload=' "$repo_root/src/gs/initialization_dft.f90"; then
  exit 1
fi
