...
mod=Path('/tmp/test_dg_wpw_rank_local_quadrature_mpi_mod');mod.mkdir(exist_ok=True)
exe=Path('/tmp/test_dg_wpw_rank_local_quadrature_mpi')
subprocess.run([os.environ.get('MPIFC','mpifort'),'-J',str(mod),'-I',str(mod),
  ...
  str(ROOT/'tests/dg/test_dg_wpw_rank_local_quadrature_mpi.f90'),'-o',str(exe)],check=True,cwd='/tmp')
