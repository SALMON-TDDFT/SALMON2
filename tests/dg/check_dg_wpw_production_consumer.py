...
assert "wpw_window_buffer=merge(dg_wpw_window_buffer,0,num_fragment>1)" in lcfo.replace(" ", ""), \
...
assert "evaluate_dg_wpw_core_w_support" not in volume, \
    "remote W halo values must not enter piecewise DG volume quadrature"
...
for stage in ("overlap_exchange", "ww_local", "ww_cross", "wp", "pp", "install"):
