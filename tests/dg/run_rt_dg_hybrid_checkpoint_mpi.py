#!/usr/bin/env python3
"""Check occupied-state persistence and formal v5 publication preconditions."""
...
with tempfile.TemporaryDirectory(prefix="hybrid-v5-publication-") as name:
    ...
    common=[shutil.which("mpifort"),"-cpp","-DUSE_MPI","-I",str(build),"-J",str(build),
            ...
            str(root/"src/rt/dg/rt_dg_hybrid_checkpoint_v5.f90"),str(source_path)]
    ...
    for nrank in (1,2,4,8):
        ...
        assert f"PASS v5 publication preconditions on {nrank} ranks" in good.stdout
        for mode,diagnostic in (
            ("bad_local","PASS named collective v5 publication precondition rejection"),
            ("bad_mapping","PASS named collective v5 mapping rejection before publication"),
        ):
...
print("PASS v5 publication and occupied checkpoints on 1, 2, 4, and 8 ranks")
