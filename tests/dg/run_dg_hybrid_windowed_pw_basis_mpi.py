...
with tempfile.TemporaryDirectory(prefix="hybrid-basis-") as name:
    build=Path(name);(build/"config.h").write_text("");exe=build/"hybrid_basis"
    subprocess.run([shutil.which("mpifort"),"-cpp","-DUSE_MPI","-I",str(build),"-J",str(build),
      ...
      str(root/"tests/dg/test_dg_hybrid_windowed_pw_basis_mpi.f90"),"-o",str(exe)],check=True)
    ...
    for nrank in (1,2,4,8):
      ...
      assert f"PASS hybrid windowed PW basis on {nrank} ranks" in run.stdout
      match=re.search(r"HYBRID_BASIS ranks=\d+ fingerprint=(-?\d+)",run.stdout);assert match,run.stdout
      fingerprints.append(int(match.group(1)))
    assert len(set(fingerprints))==1,fingerprints
print("PASS hybrid windowed PW basis on 1, 2, 4, and 8 ranks")
