...
with tempfile.TemporaryDirectory(prefix="hybrid-broken-volume-") as name:
    build = Path(name)
    (build / "config.h").write_text("")
    exe = build / "hybrid_broken_volume"
    subprocess.run([
        ...
        str(root / "tests/dg/test_dg_hybrid_broken_volume_mpi.f90"),
        "-o", str(exe),
    ], check=True)
    env = os.environ.copy()
    ...
    for nrank in (1, 2, 4):
        ...
        assert f"PASS hybrid broken volume on {nrank} ranks" in run.stdout
print("PASS hybrid broken volume on 1, 2, and 4 ranks")
