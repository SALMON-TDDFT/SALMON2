...
with tempfile.TemporaryDirectory(prefix="hybrid-ground-state-types-") as name:
  build=Path(name);(build/"config.h").write_text("");exe=build/"hybrid_ground_state_types"
  subprocess.run([shutil.which("mpifort"),"-cpp","-DUSE_MPI","-I",str(build),"-J",str(build),
    ...
    str(root/"tests/dg/test_dg_hybrid_ground_state_types_mpi.f90"),"-o",str(exe)],check=True)
  ...
  for nrank in (1,2,4,8):
    ...
    assert f"PASS hybrid ground-state types on {nrank} ranks" in run.stdout
    match=re.search(r"HYBRID_GROUND_STATE_TYPES ranks=\d+ fingerprint=(-?\d+)",run.stdout);assert match,run.stdout
    fingerprints.append(int(match.group(1)))
  assert len(set(fingerprints))==1,fingerprints
print("PASS hybrid ground-state types on 1, 2, 4, and 8 ranks")
