...
with tempfile.TemporaryDirectory(prefix="ow-eigenexa-") as name:
    ...
    support=build/"support.o";solver=build/"solver.o";test=build/"test.o";exe=build/"eigenexa_solver"
    ...
    subprocess.run([shutil.which("mpifort"),"-fopenmp",str(support),str(solver),str(test),
