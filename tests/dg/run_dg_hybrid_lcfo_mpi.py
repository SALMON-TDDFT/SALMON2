...
with tempfile.TemporaryDirectory(prefix="hybrid-lcfo-") as name:
  ...
  for nrank in (1,2,4):
    ...
    assert f"PASS hybrid LCFO assembly on {nrank} ranks" in run.stdout
    match=re.search(r"HYBRID_LCFO ranks=\d+ fingerprint=(-?\d+)",run.stdout);assert match,run.stdout
    fingerprints.append(int(match.group(1)))
  assert len(set(fingerprints))==1,fingerprints
print("PASS hybrid LCFO assembly on 1, 2, and 4 ranks")
