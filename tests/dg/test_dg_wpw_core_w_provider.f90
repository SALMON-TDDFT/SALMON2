program test_dg_wpw_core_w_provider
  ...
  complex(8)::coefficients(2,1),reconstructed(1),owned_only(1)
  ...
  coefficients(:,1)=[(0.5d0,0d0),(3d0,0d0)]
  call reconstruct_dg_wpw_core_w_support(owned_ids,owned_value,owned_gradient,support_ids,halo,&
    [3,2,1],7,coefficients,values,gradients,reconstructed,info)
  owned_only=values(1)*coefficients(1,:)
  if(info/=0.or.abs(reconstructed(1)-(16d0,-3d0))>1d-13.or.&
    abs(reconstructed(1)-owned_only(1))<1d0)error stop 7
  call evaluate_dg_wpw_core_w_support(owned_ids,owned_value,owned_gradient,support_ids,halo,&
