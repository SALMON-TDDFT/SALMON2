program test_rt_dg_hybrid_checkpoint_v5_reject_mpi
  use mpi
  ...
  call read_rt_dg_hybrid_checkpoint_v5(MPI_COMM_WORLD,trim(prefix),payload,ok,message)
  if(ok)call MPI_Abort(MPI_COMM_WORLD,1,ierr)
  if(rank==0)write(*,'(a,i0,2a)')'PASS v5 collective rejection ranks=',nproc,' diagnostic=',trim(message)
  call MPI_Finalize(ierr)
end program test_rt_dg_hybrid_checkpoint_v5_reject_mpi
