...
with tempfile.TemporaryDirectory(prefix="hybrid-wpw-projection-") as name:
  build=Path(name);(build/"config.h").write_text("");exe=build/"hybrid_wpw_projection"
  subprocess.run([shutil.which("mpifort"),"-cpp","-DUSE_MPI","-I",str(build),"-J",str(build),
    ...
    str(root/"tests/dg/test_dg_hybrid_wpw_projection_tile_mpi.f90"),"-o",str(exe)],check=True)
  ...
  for nrank in (1,2,4):
    ...
    assert f"PASS hybrid WPW projection tile on {nrank} ranks" in run.stdout
    match=re.search(r"HYBRID_WPW_PROJECTION_TILE ranks=\d+ fingerprint=(-?\d+)",run.stdout);assert match
    fingerprints.append(int(match.group(1)))
  assert len(set(fingerprints))==1,fingerprints
print("PASS hybrid WPW projection tile on 1, 2, and 4 ranks")
