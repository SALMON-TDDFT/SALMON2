...
with tempfile.TemporaryDirectory(prefix="hybrid-lcfo-support-") as name:
  ...
  subprocess.run([shutil.which("mpifort"),"-cpp","-DUSE_MPI","-I",str(build),"-J",str(build),
    ...
    str(root/"tests/dg/test_dg_hybrid_lcfo_support_redistribution_mpi.f90"),"-o",str(exe)],check=True)
  ...
  for nrank in (1,2,4):
    ...
    assert f"PASS hybrid LCFO support redistribution on {nrank} ranks" in run.stdout
    match=re.search(r"HYBRID_LCFO_SUPPORT ranks=\d+ fingerprint=(-?\d+)",run.stdout);assert match
    fingerprints.append(int(match.group(1)))
  assert len(set(fingerprints))==1,fingerprints
print("PASS hybrid LCFO support redistribution on 1, 2, and 4 ranks")
