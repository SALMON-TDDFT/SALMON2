...
with tempfile.TemporaryDirectory(prefix="hybrid-continuation-controller-") as name:
    build = Path(name)
    (build / "config.h").write_text("")
    exe = build / "hybrid_continuation_controller"
    subprocess.run([
    ...
    for nrank in (1, 2, 4, 8):
        ...
        assert f"PASS hybrid continuation controller on {nrank} ranks" in run.stdout
print("PASS hybrid continuation controller on 1, 2, 4, and 8 ranks")
