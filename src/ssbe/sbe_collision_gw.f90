...
contains
  ...
  subroutine load_gw_rate(filename, nb, nk, gamma_nk, icomm)
    use communication, only: comm_get_groupinfo, comm_bcast
    implicit none
    character(*), intent(in) :: filename
    integer,      intent(in) :: nb, nk
    real(8),      intent(out) :: gamma_nk(nb, nk)
    integer,      intent(in) :: icomm        ! < 0 => no MPI (standalone test)
    integer :: irank, nproc, fh, ios, in, ik
    ...
    irank = 0
    if (icomm >= 0) call comm_get_groupinfo(icomm, irank, nproc)
    ...
    if (icomm >= 0) call comm_bcast(gamma_nk, icomm, 0)
