...
subroutine wrapper_broyden(mg,system,srho_s,iter,icomm,mixing)
  use structures, only: s_rgrid,s_dft_system,s_scalar,s_mixing
  use broyden_sub
  implicit none
  type(s_rgrid) :: mg
  ...
  if(system%nspin==1)then
    ...
    call broyden(vecr,vecr_in,vecr_out,mg%num(1)*mg%num(2)*mg%num(3),  &
                 iter,mixing%num_rho_stock,mixing%num_rho_stock,icomm)
