...
module band_dft_sub
  ...
  private
  public :: init_band_dft
  public :: set_vec_k_band_dft
  public :: write_band_dft
...
contains
  ...
  subroutine set_vec_k_band_dft( iter_band_kpt, system, band, info )
    ...
    real(8) :: b_inv(3,3),kpt_reduced(3)

    b_inv = transpose( system%primitive_a )/(2.0d0*acos(-1.0d0))
    ...
    if( comm_is_root(nproc_id_global) ) then
      ...
      do ik=1,system%nk
        kpt_reduced(:) = matmul( b_inv, system%vec_k(:,ik) )
        write(*,30) ik, kpt_reduced(:), system%vec_k(:,ik)
        write(100,30) ik, kpt_reduced(:), system%vec_k(:,ik)
      end do
      close(100)
    end if
...
30  format(1x,i3,2x,3f10.5,2x,3f10.5)
  ...
  end subroutine set_vec_k_band_dft

  subroutine write_band_dft(system,energy)
...
9   iformat=0 ! 3 data
  ...
  subroutine get_band_kpt( kpt, nref_band, system )
    ...
    if ( allocated(ndiv_segment) ) then
      if ( comm_is_root(nproc_id_global) ) then
        write(*,*) "k points are generated by user setting"
      end if
      do ik=1,size(kpt_,2)
