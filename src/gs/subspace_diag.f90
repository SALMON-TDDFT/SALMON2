...
!
module subspace_diag_sub
  implicit none

contains

subroutine subspace_diag(mg,info_2,spsi_2,elp3,ilsda,nproc_ob,iparaway_ob,iobnum,itotmst,k_sta,k_end,mst,ifmst,hvol,  &
  ...
  !$OMP parallel do private(iz,iy,ix)
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            stpsi%rwf(ix,iy,iz,1,1,1,1)=spsi_2%rwf(ix,iy,iz,is,job_myob-(is-1)*info_2%numo,1,1)
          end do
          end do
          end do
  ...
  !$OMP parallel do reduction(+:rbox) private(iz,iy,ix)
            ...
            do iy=mg%is(2),mg%ie(2)
            do ix=mg%is(1),mg%ie(1)
              rbox=rbox+spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1)*htpsi(ix,iy,iz)
            end do
            end do
            end do
  ...
  !$OMP parallel do private(iz,iy,ix)
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            psi_box(ix,iy,iz,job)=spsi_2%rwf(ix,iy,iz,is,job-(is-1)*info_2%numo,1,1)
            spsi_2%rwf(ix,iy,iz,is,job-(is-1)*info_2%numo,1,1)=0.d0
          end do
          end do
          end do
        end if
  ...
  !$OMP parallel do private(iz,iy,ix)
            ...
            do iy=mg%is(2),mg%ie(2)
            do ix=mg%is(1),mg%ie(1)
              spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1)=   &
                spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1)  &
  ...
  !$OMP parallel do reduction(+:rbox) private(iz,iy,ix)
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            rbox=rbox+abs(spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1))**2
          end do
          end do
          end do
  ...
  !$OMP parallel do private(iz,iy,ix)
          ...
          do ix=mg%is(1),mg%ie(1)
            spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1)=   &
              spsi_2%rwf(ix,iy,iz,is,iob-(is-1)*info_2%numo,1,1)/sqrt(rbox1*Hvol)
          end do
          end do
          end do
        end if
