...
subroutine sgscg(mg,info,spsi,iflag,itotmst,mst,hvol,ilsda,nproc_ob,nproc_ob_spin,iparaway_ob,elp3, &
  ...
  !$OMP parallel do private(iz,iy,ix) collapse(2)
    ...
    !$omp parallel do private(iz,iy,ix) collapse(2) reduction(+ : sum0)
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            sum0=sum0+spsi%rwf(ix,iy,iz,1,job,1,1)*rgk_ob(ix,iy,iz,iob)
          end do
          end do
          end do
    ...
    !$omp parallel do private(iz,iy,ix) collapse(2)
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            rgk_ob(ix,iy,iz,iob)=rgk_ob(ix,iy,iz,iob)-sum_obmat1(iob,job)*spsi%rwf(ix,iy,iz,1,job,1,1)
          end do
          end do
          end do
        end do
      ...
      do iob=iobsta(is),iobend(is)
        ...
        call check_corrkob(iob,1,icorr_iob,ilsda,nproc_ob,iparaway_ob,info%ik_s,info%ik_e,nproc_ob_spin,mst)
        do job=iobsta(is),iob-1
          call calc_myob(job,job_myob,ilsda,nproc_ob,iparaway_ob,itotmst,nproc_ob_spin,mst,info%numo)
          call check_corrkob(job,1,icorr_job,ilsda,nproc_ob,iparaway_ob,info%ik_s,info%ik_e,nproc_ob_spin,mst)
          if(icorr_job==1)then
    !$omp parallel do private(iz,iy,ix) collapse(2)
            do iz=mg%is(3),mg%ie(3)
            do iy=mg%is(2),mg%ie(2)
            do ix=mg%is(1),mg%ie(1)
              rmatbox_m(ix,iy,iz)=spsi%rwf(ix,iy,iz,1,job_myob,1,1)
            end do
            end do
            end do
          end if
  ...
  !$OMP parallel do private(iz,iy,ix) collapse(2)
    ...
    do iy=mg%is(2),mg%ie(2)
    do ix=mg%is(1),mg%ie(1)
      spsi%rwf(ix,iy,iz,1,iob,1,1)=rxk_ob(ix,iy,iz,iob)/sqrt(sum_ob0(iob_allob))
    end do
    end do
    end do
  end do
