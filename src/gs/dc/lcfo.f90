...
module lcfo
  ...
  type s_dc_fragment_cache
    ...
    real(8),allocatable :: f_basis(:,:,:,:,:)
  end type s_dc_fragment_cache
...
! yn_conventional_from_dcdft==y : conventional calculation but wavefunctions are reconstructed from DC-LCFO data
  subroutine init_conventional_from_dcdft(lg,mg,system,info,spsi)
    ...
    type(s_dc_fragment_cache),allocatable :: frag_cache(:)
    ...
    do ispin=1,nspin
      do io=info%io_s,info%io_e
        if(allocated(spsi%rwf)) then
          ...
          do iy=mg%is(2),mg%ie(2)
          do ix=mg%is(1),mg%ie(1)
            spsi%zwf(ix,iy,iz,ispin,io,1,1) = 0d0
          end do
          end do
          end do
        end if
      ...
      do ifrag_blk_s=1,n_frag_local,frag_cache_batch_size
        ...
        call load_fragment_cache_batch(local_frag_ids, ifrag_blk_s, ifrag_blk_e, n_frag, nspin, nstate_tot, lg%num, frag_cache)
          ...
          do io_blk=1,nblk
            ...
            if(info%io_s <= io .and. io <= info%io_e) then
              if(allocated(spsi%rwf)) then
                ...
                do iy=mg%is(2),mg%ie(2)
                do ix=mg%is(1),mg%ie(1)
                  spsi%rwf(ix,iy,iz,ispin,io,1,1) = spsi%rwf(ix,iy,iz,ispin,io,1,1) + wrk2(ix,iy,iz,io_blk)
                end do
                end do
                end do
              else if(allocated(spsi%zwf)) then
                ...
                do iy=mg%is(2),mg%ie(2)
                do ix=mg%is(1),mg%ie(1)
                  spsi%zwf(ix,iy,iz,ispin,io,1,1) = spsi%zwf(ix,iy,iz,ispin,io,1,1) + dcmplx(wrk2(ix,iy,iz,io_blk))
                end do
                end do
                end do
              end if
        ...
        call free_fragment_cache_batch(frag_cache)
  ...
  subroutine load_fragment_cache_batch(local_frag_ids, ifrag_blk_s, ifrag_blk_e, n_frag, nspin, nstate_tot, lgnum_tot, frag_cache)
    use filesystem, only: get_filehandle
    implicit none
    ...
    type(s_dc_fragment_cache),intent(inout) :: frag_cache(:)
    ...
    do ifrag_local=1,size(frag_cache)
      ...
      if( any( lgnum_tmp /= lgnum_tot ) ) stop "data_dcdft: input mismatch (lg)"
  ...
  end subroutine load_fragment_cache_batch

  subroutine free_fragment_cache_batch(frag_cache)
    implicit none
    type(s_dc_fragment_cache),intent(inout) :: frag_cache(:)
    integer :: ifrag_local
  ...
  end subroutine free_fragment_cache_batch
