module dc_fragment_geometry
  ...
  public :: find_fragment_face_neighbor
  public :: fragment_direction_tag
...
contains
  ...
  subroutine find_fragment_face_neighbor(fragment_origin, fragment_extent, total_extent, &
                                         current_fragment, dvec, neighbor_fragment, match_count)
    integer, intent(in) :: fragment_origin(:,:), fragment_extent(:,:), total_extent(3)
    integer, intent(in) :: current_fragment, dvec(3)
    integer, intent(out) :: neighbor_fragment, match_count
    integer :: candidate, axis
    logical :: matches_face

    neighbor_fragment = -1
    match_count = 0
    if (size(fragment_origin,1) /= 3 .or. size(fragment_extent,1) /= 3) return
    if (size(fragment_origin,2) /= size(fragment_extent,2)) return
    if (current_fragment < 1 .or. current_fragment > size(fragment_origin,2)) return
    if (any(total_extent <= 0) .or. any(abs(dvec) > 1) .or. all(dvec == 0)) return
    if (any(fragment_extent(:,current_fragment) <= 0)) return
    if (any(fragment_origin(:,current_fragment) < 0) .or. &
        any(fragment_origin(:,current_fragment) + fragment_extent(:,current_fragment) > total_extent)) return

    do candidate = 1, size(fragment_origin,2)
      if (candidate == current_fragment) cycle
      if (any(fragment_extent(:,candidate) <= 0)) cycle
      if (any(fragment_origin(:,candidate) < 0) .or. &
          any(fragment_origin(:,candidate) + fragment_extent(:,candidate) > total_extent)) cycle
      matches_face = .true.
      do axis = 1, 3
        select case (dvec(axis))
        case (1)
          matches_face = matches_face .and. modulo(fragment_origin(axis,candidate) - &
            fragment_origin(axis,current_fragment) - fragment_extent(axis,current_fragment), &
            total_extent(axis)) == 0
        case (-1)
          matches_face = matches_face .and. modulo(fragment_origin(axis,candidate) + &
            fragment_extent(axis,candidate) - fragment_origin(axis,current_fragment), &
            total_extent(axis)) == 0
        case (0)
          matches_face = matches_face .and. &
            fragment_origin(axis,candidate) == fragment_origin(axis,current_fragment) .and. &
            fragment_extent(axis,candidate) == fragment_extent(axis,current_fragment)
        end select
      end do
      if (matches_face) then
        match_count = match_count + 1
        neighbor_fragment = candidate
      end if
    end do
  end subroutine find_fragment_face_neighbor

  subroutine fragment_direction_tag(sender_fragment, n_frag, dvec, tag, valid, tag_upper_bound)
    integer, intent(in) :: sender_fragment, n_frag, dvec(3)
    integer, intent(out) :: tag
    logical, intent(out) :: valid
    integer, intent(in), optional :: tag_upper_bound
    integer :: direction_code, upper_bound
    integer(8) :: candidate_tag

    tag = -1
    valid = .false.
    upper_bound = 32767 ! MPI requires MPI_TAG_UB to be at least this value.
    if (present(tag_upper_bound)) upper_bound = tag_upper_bound
    if (n_frag < 1 .or. sender_fragment < 1 .or. sender_fragment > n_frag) return
    if (any(abs(dvec) > 1) .or. all(dvec == 0) .or. upper_bound < 0) return
    direction_code = (dvec(1)+1)*9 + (dvec(2)+1)*3 + dvec(3)+1
    candidate_tag = int(direction_code,8)*int(n_frag,8) + int(sender_fragment-1,8)
    if (candidate_tag > int(upper_bound,8) .or. candidate_tag > int(huge(tag),8)) return
    tag = int(candidate_tag)
    valid = .true.
  end subroutine fragment_direction_tag
