...
module dg_hybrid_continuation_controller
  ...
  type,public::s_dg_hybrid_controller
    logical::valid=.false.,trial_active=.false.,trace_valid=.false.
...
contains
  ...
  subroutine initialize_dg_hybrid_controller(comm,controls,accepted_lambda,accepted_state,face_count,controller,ok,message)
    ...
    local_bad=merge(0,1,valid_controls(controls).and.accepted_lambda>=0d0.and.accepted_lambda<=1d0.and.&
      ieee_is_finite(accepted_lambda).and.face_count>0.and.valid_state(accepted_state))
    ...
    controller%valid=.true.;controller%trial_active=.false.;controller%trace_valid=.true.;ok=.true.;message=''
#else
  ...
  subroutine observe_dg_hybrid_inner_residuals(comm,controller,residuals,reject_requested,ok,message)
...
#ifdef USE_MPI
    integer::local_bad,global_bad,ierr
    real(real64)::growth,numerical_floor
    ...
    numerical_floor=100d0*tiny(1d0);reject_requested=.false.
    if(controller%inner_sample_count>0)then
      growth=maxval(residuals/max(controller%previous_inner,numerical_floor))
    ...
    controller%previous_inner=residuals;controller%inner_sample_count=controller%inner_sample_count+1
    call collective_logical_or(comm,reject_requested,ierr)
    ok=ierr==MPI_SUCCESS;if(ok)then;message='';else;message='inner residual decision reduction failed';endif
#else
  ...
  subroutine decide_dg_hybrid_stage(comm,controller,state,report,accept,ok,message)
    ...
    integer::local_integer,global_integer,ierr
    ...
    local_accept=controller%valid.and.controller%trial_active.and.valid_state(state).and.report%iteration>=1.and.&
      report%iteration<=controller%controls%iteration_limit.and.all(ieee_is_finite(report%residuals)).and.&
      all(report%residuals<=stage_tolerances).and.ieee_is_finite(report%projector_overlap).and.&
    ...
    controller%accepted_state=state;controller%accepted_lambda=controller%trial_lambda
    if(report%gap_shrinking)then
      controller%step=max(controller%controls%minimum_step,controller%step*controller%controls%shrink_factor)
    else if(report%iteration<=controller%controls%iteration_limit/2)then
      controller%step=min(controller%controls%maximum_step,controller%step*controller%controls%growth_factor)
    endif
    controller%trial_active=.false.;controller%trace_valid=.false.;controller%growth_streak=0
#else
  ...
  subroutine reject_dg_hybrid_trial(comm,controller,state,reason,ok,message)
    ...
    controller%trace_valid=.false.;controller%inner_sample_count=0;controller%growth_streak=0
  ...
  logical function valid_controls(controls) result(valid)
    type(s_dg_hybrid_controller_controls),intent(in)::controls
    valid=controls%minimum_step>0d0.and.controls%initial_step>=controls%minimum_step.and.&
...
#ifdef USE_MPI
  subroutine collective_logical_or(comm,value,ierr)
    integer,intent(in)::comm;logical,intent(inout)::value;integer,intent(out)::ierr
    integer::local_value,global_value
    local_value=merge(1,0,value);call MPI_Allreduce(local_value,global_value,1,MPI_INTEGER,MPI_MAX,comm,ierr)
    value=global_value/=0
  end subroutine collective_logical_or
#endif
