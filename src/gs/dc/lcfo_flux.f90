    ...
    ! DG surface jump/average/penalty terms on face-neighbor blocks.
      if(dc%id_frag==0) then
        ...
        do i_halo=1,n_halo
          ...
          do ispin=1,nspin
!$omp parallel do private(io,jo,face_pt,ix_face,iy_face,iz_face,u_l,v_l,dnu_l,dnv_l,term_sum,term_face) schedule(static)
          do io=1,n_basis(dc%i_frag,ispin)
          do jo=1,n_basis(dc%i_frag,ispin)
            term_sum = 0d0
            face_pt = 0
...
!$omp end parallel do
          ...
          do ispin=1,nspin
!$omp parallel do private(io,jo,face_pt,ix_face,iy_face,iz_face,v_l,dnv_l,u_r,dnu_r,term_sum,term_face) schedule(static)
...
!$omp end parallel do
    ...
    subroutine diag_eigenexa
      ...
      do ispin=1,nspin
            ...
            do ix_loc=ix_s,ix_e
              ...
              do i_halo=1,n_halo
                if( ifrag_x == jfrag_halo(i_halo) .and. ifrag_y == ifrag ) then
                  h_div(ix_loc,iy_loc) = h_div(ix_loc,iy_loc) + h(io_x,io_y,i_halo)
                else if( ifrag_x == ifrag .and. ifrag_y == jfrag_halo(i_halo) ) then
                  h_div(ix_loc,iy_loc) = h_div(ix_loc,iy_loc) + h(io_y,io_x,i_halo)
                end if
              end do ! i_halo
            end do ! ix_loc
          end do ! iy_loc
        ...
        deallocate(h_div,v_div,v_tmp1,v_tmp2,io_array,ifrag_array)
        call eigen_free()
      end do ! ispin
