...
module dcdft
  ...
  implicit none
  interface calc_vlocal_fragment_dcdft
    module procedure calc_vlocal_fragment_dcdft_meta
    module procedure calc_vlocal_fragment_dcdft_copy
  end interface
contains
  ...
  contains
    ...
    ! initialization for the total system
      ...
      & stencil_dummy,dc%fg_tot,dc%poisson_tot,dc%srg_tot,dc%srg_scalar_tot,ofile_dummy,unfold_dummy)
...
!write(*,'(a,5i10)') " i_frag,id_F,isize_F,id,isize",dc%i_frag,id_frag,isize_frag,dc%id_tot,dc%isize_tot
    ...
    subroutine init_fragment
      ...
      & file_atom_coor_frag, yn_dc_fragment_optimization
      implicit none
...
!===================================================================================================================================
  ...
  subroutine calc_vlocal_fragment_dcdft_meta(system,mg,info,stencil,xc_func,srg_scalar,srg,rho_s, &
  ...
  end subroutine calc_vlocal_fragment_dcdft_meta

  ! Copy the already assembled total-system local potential into a fragment.
  ! The SOI wrapper uses this overload after adding its spinor contribution.
  subroutine calc_vlocal_fragment_dcdft_copy(nspin,mg,vloc,dc)
    use structures
    use communication, only: comm_summation
    implicit none
    integer,intent(in) :: nspin
    ...
    type(s_dcdft),intent(in) :: dc
    integer :: ix,iy,iz,ispin,ix_tot,iy_tot,iz_tot
    real(8),allocatable :: total_local(:,:,:,:),total_local_sum(:,:,:,:)

    allocate(total_local(dc%lg_tot%num(1),dc%lg_tot%num(2),dc%lg_tot%num(3),nspin), &
      total_local_sum(dc%lg_tot%num(1),dc%lg_tot%num(2),dc%lg_tot%num(3),nspin))
    total_local=0d0
    do ispin=1,nspin
    do iz=dc%mg_tot%is(3),dc%mg_tot%ie(3)
    do iy=dc%mg_tot%is(2),dc%mg_tot%ie(2)
    do ix=dc%mg_tot%is(1),dc%mg_tot%ie(1)
      total_local(ix,iy,iz,ispin)=dc%vloc_tot(ispin)%f(ix,iy,iz)
    enddo
    enddo
    enddo
    enddo
    call comm_summation(total_local,total_local_sum,size(total_local),dc%icomm_tot)
    do ispin=1,nspin
    do iz=mg%is(3),mg%ie(3)
    ...
    do ix=mg%is(1),mg%ie(1)
      ...
      vloc(ispin)%f(ix,iy,iz)=total_local_sum(ix_tot,iy_tot,iz_tot,ispin)
    enddo
    enddo
    enddo
    enddo
  end subroutine calc_vlocal_fragment_dcdft_copy
