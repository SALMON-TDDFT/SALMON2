    ...
    subroutine init_lcfo
      ...
      do lz=-nh(3),nh(3)
        ...
        if(matches_dst/=1.or.matches_src/=1)then
          write(*,'(a,i0,a,3(1x,i0),a,2(1x,i0))') 'DC-LCFO(SOI) fragment ',dc%i_frag,&
            ' has missing/ambiguous halo direction',halo(i)%dvec,'; dst/src matches',matches_dst,matches_src
          error stop 'DC-LCFO(SOI): fragment face topology is not unique'
        end if
      ...
      do i=1,n_halo
        ! Halo slices retain the local SOI core dimensions established above.
        ! Tangential extents are guaranteed equal by face-topology matching.
        do n=1,3
    ...
    subroutine calc_hamiltonian_matrix
      ...
      if(dc%id_frag==0) then
        do i_halo=1,n_halo
          ...
          if(.not.valid_send_tag)error stop 'DC-LCFO(SOI): send halo tag exceeds portable MPI_TAG_UB'
          ...
          if(.not.valid_recv_tag)error stop 'DC-LCFO(SOI): receive halo tag exceeds portable MPI_TAG_UB'
