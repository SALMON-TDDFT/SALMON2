...
contains
  ...
  subroutine solve_dg_hybrid_fragment_basis_legacy(comm,basis,nstate,occupations,core_mask,point_weights,&
      apply_h,apply_s,tolerance,&
      coefficients,eigenvalues,core_density,core_electron_count,maximum_residual,orthogonality_defect,&
      workspace_peak_bytes,fingerprint,ok,message)
    integer,intent(in)::comm,nstate
    type(s_dg_hybrid_fragment_basis),intent(in)::basis
    ...
    integer(int64),allocatable::all_ids(:),ordered_ids(:)
    ...
    local_bad=0
    if(any(occupations<0d0).or.any(.not.ieee_is_finite(point_weights)).or.&
    ...
    allocate(all_ids(nbasis),ordered_ids(nbasis))
    ...
    ordered_ids=all_ids;call sort_ids(ordered_ids)
    if(any(ordered_ids<=0_int64))local_bad=1
    do i=2,nbasis;if(ordered_ids(i)==ordered_ids(i-1))local_bad=1;enddo
    ...
    do j=1,nbasis;do target=1,global_point_count
      local_value=(0d0,0d0)
      do i=1,nowned
        position=findloc(ordered_ids,basis%global_ids(i),dim=1);if(position/=j)cycle
        do p=1,npoint
    ...
    do i=1,nowned
      position=findloc(ordered_ids,basis%global_ids(i),dim=1);coefficients(i,:)=hmat(position,1:nstate)
    enddo
  ...
  contains
    subroutine sort_ids(ids)
      ...
      integer(int64)::key
      do right=2,size(ids)
        key=ids(right);left=right-1
        do while(left>=1)
          if(ids(left)<=key)exit
          ids(left+1)=ids(left);left=left-1
        enddo
        ids(left+1)=key
      enddo
  ...
  end subroutine solve_dg_hybrid_fragment_basis_legacy
