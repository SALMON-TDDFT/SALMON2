...
module dg_hybrid_continuation_scf
  ...
  private
  abstract interface
    subroutine volume_callback(lambda,density,ok)
      import real64
      real(real64),intent(in)::lambda,density(:);logical,intent(out)::ok
  ...
  public::run_dg_hybrid_coupled_fixed_points
contains
  subroutine run_dg_hybrid_coupled_fixed_points(comm,continuation,controls,seed_state,face_count,&
    ...
    integer::iteration,ierr,local_bad,global_bad
    ...
    local_bad=merge(0,1,continuation%valid.and.continuation%lambda==0d0.and.&
      ...
      size(continuation%seed_density)>0.and.size(continuation%seed_density)==size(seed_state%density).and.&
    ...
    state=controller%accepted_state;lambda=1d0;input_density=state%density;input_trace=state%trace
    ...
    state%density=input_density;state%trace_cache_valid=.true.;final_state=state;final_lambda=1d0
    ok=.true.;message=''
  contains
    subroutine converge_current(use_controller,converged,rejected_trial,fatal)
      ...
      do iteration=1,maximum_inner_iterations
        ...
        if(.not.callback_ok)then;message='coupled continuation callback failed';fatal=.true.;return;endif
    ...
    subroutine execute_callbacks(inner_iteration,callbacks_ok)
      ...
      call build_volume(lambda,input_density,callback_ok);call callback_consensus(callback_ok,global_bad,ierr)
