...
contains
  ...
  subroutine gscg_rwf_so(ncg,mg,system,info,stencil,ppg,vlocal,srg,spsi,cg)
    use structures
    use timer
    ...
    integer :: iter
    real(8),dimension(system%nspin,system%no) :: summ,xkxk,xkHxk,xkHpk,pkHpk,gkgk,uk,ev,cx,cp,zs
    ...
    Iteration : do iter=1,Ncg
      ...
      if(iter==1)then
        ...
        do ispin=1,nspin
          if (abs(gkgk(ispin,io)) > 1d-16) then
            uk(ispin,io) = summ(ispin,io) / gkgk(ispin,io)
          else
            uk(ispin,io) = 0d0
          end if
        end do
      ...
      do iy=is(2),ie(2)
        cg%pko%rwf(is(1):ie(1),iy,iz,ispin,io,1,1) = &
      &   cg%pko%rwf(is(1):ie(1),iy,iz,ispin,io,1,1) / sqrt(summ(ispin,io))
      end do
      end do
      end do
      end do
  ...
  contains
    ...
    subroutine inner_product(mg,system,info,psi1,psi2,rbox)
      ...
      rbox2 = 0.d0
      !$OMP parallel do private(io,ispin,sum0,iz,iy,ix)
      do io=info%io_s,info%io_e
        ...
        rbox2(1,io) = sum0 * system%hvol
        rbox2(2,io) = rbox2(1,io)
      end do
      call timer_end(LOG_GSCG_ISOLATED_CALC)
...
!===================================================================================================================================
  ...
  subroutine gscg_zwf_so(ncg,mg,system,info,stencil,ppg,vlocal,srg,spsi,cg)
    use structures
    use timer
    use hamiltonian, only: hpsi
    use communication, only: comm_summation
    ...
    integer :: iter
    complex(8),dimension(system%nspin,system%no,system%nk) :: summ,xkxk,xkHxk,xkHpk,pkHpk,gkgk,uk,ev,cx,cp,zs
...
#ifdef USE_OPENACC
    !$acc data copy(spsi%zwf) &
    !$acc      create(xkxk,xkHxk,pkHpk,xkHpk,cg%xk%zwf,cg%hxk%zwf,cg%pk%zwf,cg%hwf%zwf,cg%pko%zwf,cg%gk%zwf,cg%pre_gk%zwf,uk,summ,gkgk,zs,ev,cx,cp)
#endif
    ...
    Iteration : do iter=1,Ncg
      ...
      if(iter==1)then
        ...
        do ispin=1,nspin
          if (abs(gkgk(ispin,io,ik)) > 1d-16) then
            uk(ispin,io,ik) = summ(ispin,io,ik) / gkgk(ispin,io,ik)
          else
            uk(ispin,io,ik) = 0d0
          end if
        end do
      ...
      do iy=is(2),ie(2)
        cg%pko%zwf(is(1):ie(1),iy,iz,ispin,io,ik,1) = &
      & cg%pko%zwf(is(1):ie(1),iy,iz,ispin,io,ik,1) / sqrt(summ(ispin,io,ik))
      end do
      end do
      end do
      end do
...
end module Conjugate_Gradient_so
