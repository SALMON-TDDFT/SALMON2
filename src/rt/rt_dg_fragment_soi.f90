...
module rt_dg_fragment_soi
  ...
  ! NOTE: This module is structurally parallel to rt_dg_fragment (non-SOI variant).
  !       The HSE-ACE cache infrastructure is intentionally identical; any bug fix
  !       here must also be applied to rt_dg_fragment and vice versa.
  ...
  subroutine read_fragment_basis_data(dg_frag, bdir_frag)
    ...
    if (env_status == 0 .and. env_len > 0) then
      ...
      if (ienv == 0 .and. n_mat_cap_env >= 1) then
        ...
        if (dg_frag%id == 0) then
          write(*,'(1x,a,i0)') "[INFO] SALMON_DG_NMAT_CAP applied: ", n_mat_cap
        end if
      else
        if (dg_frag%id == 0) then
    ...
    do ifrag = dg_frag%ifrag_start, dg_frag%ifrag_end
      ...
      ! Keep spin-1 basis in both complex storage (phi_frag_c) and legacy real storage (phi_frag).
      block
...
#include "rt_dg_fragment_hamiltonian_soi.f90"
...
#include "rt_dg_density_reconstruct.f90"
  ...
  subroutine add_exact_exchange_hse(dg_frag, system, H_mat_spin, ifrag, ispin)
    ...
    hvol = system%hvol
    ! Use fragment-local interior indices (1:nxyz_domain); see rt_dg_fragment.f90.
    is = 1
