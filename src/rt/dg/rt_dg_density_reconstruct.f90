  subroutine calculate_density_from_fragments(dg_frag, system, mg, rho, rho_s, itt_debug)
    ...
    real(8) :: time_cache_pw_refresh, time_cache_phi_block_refresh, time_cache_phase_block_refresh
    ...
    character(32) :: env_density_moment_trace
    ...
    logical, save :: cfg_enable_density_timing = .false.
    logical, save :: cfg_enable_density_stage_contrib_trace = .false.
    logical, save :: cfg_enable_density_moment_trace = .false.
    character(32), save :: cfg_env_rho_mix_mode = 'legacy'
    integer, save :: cfg_rho_mix_mode_kind = 0
    logical, save :: cfg_enable_rho_mix_trace = .false.
    logical, save :: cfg_enable_rho_mix_raw_trace = .false.
    logical, save :: cfg_enable_rho_mix_grid_compare = .false.
    logical, save :: cfg_enable_ifrag_compare_trace = .false.
    logical, save :: cfg_enable_fp_decomp_audit = .false.
    logical, save :: cfg_enable_fp_phase_fix = .false.
    logical, save :: cfg_enable_tf_occmap_only = .false.
    integer, save :: cfg_n_occmap_trace_itt = 0
    integer, save :: cfg_occmap_trace_itts(max_occmap_trace_itt) = 0
    ...
    integer, allocatable, save :: cmp_tf_m2_ref_static(:,:)
    integer, allocatable, save :: cmp_tf_track_m2(:,:,:)
    integer, allocatable, save :: cmp_tf_track_itt(:)
    ...
    time_cache_phase_block_refresh = 0.0d0
    ...
    if (.not. density_env_initialized) then
      ...
      call get_environment_variable('SALMON_DG_DENSITY_TIMING', env_density_timing, status=env_status)
      if (env_status == 0) then
        select case (adjustl(trim(env_density_timing)))
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_density_timing = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_density_timing = .false.
        end select
      end if
      call get_environment_variable('SALMON_DG_DENSITY_STAGE_CONTRIB', env_density_stage_contrib, status=env_status)
      if (env_status == 0) then
        select case (adjustl(trim(env_density_stage_contrib)))
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_density_stage_contrib_trace = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_density_stage_contrib_trace = .false.
        end select
      end if
      call get_environment_variable('SALMON_DG_DENSITY_MOMENT_TRACE', env_density_moment_trace, status=env_status)
      if (env_status == 0) then
        select case (adjustl(trim(env_density_moment_trace)))
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_density_moment_trace = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_density_moment_trace = .false.
        end select
      end if
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_rho_mix_trace = .true.
        end select
      end if
      cfg_enable_rho_mix_raw_trace = cfg_enable_rho_mix_trace
      cfg_enable_rho_mix_grid_compare = cfg_enable_rho_mix_trace
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_rho_mix_raw_trace = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_rho_mix_raw_trace = .false.
        end select
      end if
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_rho_mix_grid_compare = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_rho_mix_grid_compare = .false.
        end select
      end if
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_ifrag_compare_trace = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_ifrag_compare_trace = .false.
        end select
      end if
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_fp_decomp_audit = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_fp_decomp_audit = .false.
        end select
      end if
      if (cfg_enable_fp_decomp_audit) then
        cfg_enable_rho_mix_raw_trace = .true.
      end if
      ...
      if (env_status == 0) then
        ...
        case('1','y','Y','yes','YES','true','TRUE','on','ON')
          cfg_enable_tf_occmap_only = .true.
        case('0','n','N','no','NO','false','FALSE','off','OFF')
          cfg_enable_tf_occmap_only = .false.
        end select
      end if
      ...
      if (env_status == 0) then
        ...
        do while (occmap_trace_pos <= len_trim(env_tf_occmap_itts_work))
          ...
          if (occmap_trace_ios == 0) then
            if (cfg_n_occmap_trace_itt < max_occmap_trace_itt) then
              cfg_n_occmap_trace_itt = cfg_n_occmap_trace_itt + 1
              cfg_occmap_trace_itts(cfg_n_occmap_trace_itt) = occmap_trace_val
            end if
          end if
    ...
    enable_density_timing = cfg_enable_density_timing
    enable_density_stage_contrib_trace = cfg_enable_density_stage_contrib_trace
    enable_density_moment_trace = cfg_enable_density_moment_trace
    env_rho_mix_mode = cfg_env_rho_mix_mode
    rho_mix_mode_kind = cfg_rho_mix_mode_kind
    enable_rho_mix_trace = cfg_enable_rho_mix_trace
    enable_rho_mix_raw_trace = cfg_enable_rho_mix_raw_trace
    enable_rho_mix_grid_compare = cfg_enable_rho_mix_grid_compare
    enable_ifrag_compare_trace = cfg_enable_ifrag_compare_trace
    enable_fp_decomp_audit = cfg_enable_fp_decomp_audit
    enable_fp_phase_fix = cfg_enable_fp_phase_fix
    enable_tf_occmap_only = cfg_enable_tf_occmap_only
    n_occmap_trace_itt = cfg_n_occmap_trace_itt
    occmap_trace_itts(:) = 0
    if (cfg_n_occmap_trace_itt > 0) then
      occmap_trace_itts(1:cfg_n_occmap_trace_itt) = cfg_occmap_trace_itts(1:cfg_n_occmap_trace_itt)
    end if
    ...
    ! Full mixed-state coefficients are only needed by diagnostics that inspect
    ! individual occupied states.  The production density path can build local
    ! occupied-state slices and reduce the final density matrix over icomm_frag.
    need_full_coef_mix_spin = enable_rho_mix_trace .or. enable_rho_mix_grid_compare .or. &
                              enable_tf_occmap_only .or. enable_fp_decomp_audit
    need_mode_diagnostics = enable_tf_occmap_only .or. enable_fp_decomp_audit
    ...
    call density_checkpoint('env/config done')
    ...
    if (enable_ifrag_compare_trace) then
    ...
    call density_checkpoint('before-work-alloc')
    ...
    call density_checkpoint('work-alloc done')
    ...
    call density_checkpoint('rhobf/sendrecv-headers alloc done')
    ...
    if (need_rhobf_box_cache) then
      call density_checkpoint('rhobf-map-cache rebuild start')
      ...
      call density_checkpoint('rhobf-map-cache rebuild done')
    end if

    call density_checkpoint('before-rhobf-zero')
    rho%f = 0.0d0
    rho_bf(:, :, :) = 0.0d0
    if (system%nspin > 1) rho_s_bf(:, :, :, :) = 0.0d0
    rho_blk_reduced(:) = 0.0d0
    call density_checkpoint('rhobf-zero done')
    ...
    if (enable_tf_occmap_only) then
      if (.not. allocated(cmp_tf_m2_ref_static)) allocate(cmp_tf_m2_ref_static(max_occmap_ref_occ, n_focus_gid))
      if (.not. allocated(cmp_tf_track_m2)) allocate(cmp_tf_track_m2(max_occmap_ref_occ, n_focus_gid, max_occmap_track_step))
      if (.not. allocated(cmp_tf_track_itt)) allocate(cmp_tf_track_itt(max_occmap_track_step))
      if (itt_tag <= 1) then
    ...
    call density_checkpoint('rho-send alloc done')
    do irank = 0, dg_frag%isize - 1
      npts = dg_frag%density_recv_map(irank)%npts
      if (npts <= 0) cycle
    end do
    call density_checkpoint('rho-recv alloc done')
    ...
    if (n_pw == 0) then
      ...
      call density_checkpoint('dense-d-alloc done')
    end if
    ...
    if (enable_density_phi_block_cache) then
      call density_checkpoint('phi-block-cache check start')
      ...
      if (need_phi_cache_alloc .or. need_phi_count_alloc .or. need_phi_cache_invalid .or. need_phi_cache_resize) then
        call density_checkpoint('phi-block-cache rebuild start')
...
!$omp end parallel do
        ...
        call density_checkpoint('phi-block-cache rebuild done')
      end if
    else
      if (allocated(dg_frag%density_phi_block_cache)) deallocate(dg_frag%density_phi_block_cache)
      if (allocated(dg_frag%density_phi_block_count)) deallocate(dg_frag%density_phi_block_count)
    ...
    call density_checkpoint('phi-block-cache section done')
    call cpu_time(t_setup1)
    ...
    call density_checkpoint('phase-block-cache section start')
    call cpu_time(t_setup0)
...
!$omp end parallel do
    ...
    call density_checkpoint('phase-block-cache section done')
    call cpu_time(t_setup1)
    if (rebuilt_phase_block_cache) time_cache_phase_block_refresh = time_cache_phase_block_refresh + (t_setup1 - t_setup0)
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a,l1,a,l1,a,l1,a,l1,a,l1,a,1pe12.4)') "        density cache: phase_block enabled=", &
        enable_density_phase_block_cache, " rebuilt=", rebuilt_phase_block_cache, " alloc=", need_phase_cache_alloc, &
        " invalid=", need_phase_cache_invalid, " resize=", need_phase_cache_resize, " dt=", time_cache_phase_block_refresh
      flush(6)
    end if
    call density_checkpoint('before-fragment-loop')
    call cpu_time(t_project0)
      ...
      do ifrag = dg_frag%ifrag_start, dg_frag%ifrag_end
        ...
        call density_checkpoint('before-dmat-zero')
        if (n_pw == 0) D_frag_re(:,:,:) = 0.0d0
        call density_checkpoint('dmat-zero done')
        if (n_pw == 0) then
          do ispin = 1, system%nspin
            nbf = dg_frag%n_basis(ifrag, ispin)
            nocc_spin = dg_frag%nocc_spin(ispin)
            ...
            call density_checkpoint('occ-cache done')
            ...
            call density_checkpoint('coef-c-zero done')
...
!$omp end parallel do
            call density_checkpoint('coef-c-fill done')
            ...
            call density_checkpoint('coef-c-reduce done')
            coef_re_full(1:nbf_max, 1:nocc_spin, ispin) = real(coef_c_full(1:nbf_max, 1:nocc_spin), kind=8)
            coef_im_full(1:nbf_max, 1:nocc_spin, ispin) = aimag(coef_c_full(1:nbf_max, 1:nocc_spin))
            call density_checkpoint('coef-realimag done')
            ...
            if (enable_density_reconstruct_trace) then
              ...
              do iprobe = 1, min(3, nbf)
                ...
                do io = 1, min(3, nocc_spin)
                  ...
                  coef_map_global_probe(iprobe, io) = 0.0d0
                  if (allocated(dg_frag%coef_global_to_local)) then
                    io_loc = dg_frag%coef_global_to_local(basis_gid_spin(iprobe, ispin), ispin)
                    if (io_loc >= 1 .and. io_loc <= size(dg_frag%coef, 1)) then
                      coef_map_global_probe(iprobe, io) = real(dg_frag%coef(io_loc, io, ispin), kind=8)
                    end if
                  end if
            ...
            call density_checkpoint('D-partial-zero done')
            if (enable_density_reconstruct_trace) then
            ...
            if (nocc_loc > 0 .and. nbf > 0) then
              ...
              call density_checkpoint('D-partial-dsyrk done')
            ...
            call density_checkpoint('D-frag-reduce/copy done')
            ...
            call density_checkpoint('D-sym start')
            ...
            call density_checkpoint('D-sym done')
            ...
            call density_checkpoint('dmat-spin done')
          end do
        else
...
!$omp end parallel do
        ...
        ! --- end D pre-pass ---
        call density_checkpoint('dmat-prepass done')
        do block_offset = first_block_offset, nblocks_ifrag - 1, block_step_blocks
          ...
          call density_checkpoint('grid-block begin')
          local_grid_count = 0
          remote_grid_count = 0
          valid_remote_grid_count = 0
          call density_checkpoint('grid-buffer prep start')
          call cpu_time(t_setup0)
          ...
          call density_checkpoint('grid-buffer prep done')
          ...
          call density_checkpoint('grid-classify done')

          if (n_pw > 0) then
            call density_checkpoint('phase-build start')
...
!$omp end parallel do
            ...
            call density_checkpoint('phase-build done')
          else
            phase_applied_total_block = 0.0d0
          end if
          ...
          call cpu_time(t_setup0)
          call density_checkpoint('phi-pack start')
          if (enable_density_phi_block_cache) then
...
!$omp end parallel do
          ...
          call density_checkpoint('phi-pack done')
          call cpu_time(t_setup1)
...
!$omp end parallel do
              ...
              if (dg_frag%is_frag_root) then
!$omp parallel do reduction(+:send_pre_block_raw) &
!$omp& private(idx_remote, igrid, owner_rank, slot, rho_contrib, spin_offset) schedule(static)
...
!$omp end parallel do
              ...
              call density_checkpoint('rho-block begin')
              ...
              if (n_pw == 0) then
                ...
                if (dg_frag%parallel_mode_orbital .and. .not. enable_density_reconstruct_trace) then
                  ...
                  call cpu_time(t_rho0)
                  rho_blk_partial(1:npt_blk) = 0.0d0
                  call density_checkpoint('rho-dmat-contract start')
                  if (nbf_frag_count > 0) then
...
!$omp end parallel do
                  ...
                  call density_checkpoint('rho-dmat-contract done')
                  ...
                  if (density_on_frag_root) then
                    call density_checkpoint('rho-dmat-root reduce start')
                ...
                do io0 = io_s_frag, io_e_frag, state_block_size
                  ...
                  if (io0 == io_s_frag) call density_checkpoint('rho-dense first-batch begin')
                  call cpu_time(t_psi0)
                  psi_blk_re(1:npt_blk, 1:nbatch) = 0.0d0
                  psi_blk_im(1:npt_blk, 1:nbatch) = 0.0d0
                  if (nbf_frag_count > 0) then
                    ...
                    call dgemm('N', 'N', npt_blk, 2*nbatch, nbf_frag_count, 1.0d0, phi_blk, grid_block_size, &
                               coef_blk_ri, nbf_max, 0.0d0, psi_blk_ri, grid_block_size)
                    if (io0 == io_s_frag) call density_checkpoint('rho-dense first-dgemm done')
                    psi_blk_re(1:npt_blk, 1:nbatch) = psi_blk_ri(1:npt_blk, 1:nbatch)
                    psi_blk_im(1:npt_blk, 1:nbatch) = psi_blk_ri(1:npt_blk, nbatch+1:2*nbatch)
                  end if
                  if (density_on_frag_root) then
...
!$omp end parallel
                ...
                call density_checkpoint('rho-dense batches done')
                if (density_on_frag_root) then
                  if (dg_frag%is_frag_root) then
                    rho_blk_accum(1:npt_blk) = rho_blk_partial(1:npt_blk)
                  else
                  ...
                  call density_checkpoint('rho-dense root copy done')
                else if (dg_frag%isize_frag > 1 .and. dg_frag%icomm_frag /= COMM_GROUP_NULL) then
                  call cpu_time(t_setup0)
                  call density_checkpoint('rho-dense reduce start')
                  call comm_summation(rho_blk_partial(1:npt_blk), rho_blk_reduced(1:npt_blk), npt_blk, dg_frag%icomm_frag)
                  rho_blk_accum(1:npt_blk) = rho_blk_reduced(1:npt_blk)
                  call density_checkpoint('rho-dense reduce done')
                  call cpu_time(t_setup1)
                  time_project_rho_reduce = time_project_rho_reduce + (t_setup1 - t_setup0)
                else
                  rho_blk_accum(1:npt_blk) = rho_blk_partial(1:npt_blk)
                  call density_checkpoint('rho-dense copy done')
                end if
                if (enable_density_reconstruct_trace .and. block_offset == first_block_offset) then
...
!$omp end parallel
                ...
                if (density_on_frag_root) then
                  ...
                  call density_checkpoint('rho-pw root copy done')
                else if (dg_frag%isize_frag > 1 .and. dg_frag%icomm_frag /= COMM_GROUP_NULL) then
                  call density_checkpoint('rho-pw reduce start')
                  call comm_summation(rho_blk_partial(1:npt_blk), rho_blk_accum(1:npt_blk), npt_blk, dg_frag%icomm_frag)
                  call density_checkpoint('rho-pw reduce done')
                else
                  rho_blk_accum(1:npt_blk) = rho_blk_partial(1:npt_blk)
                  call density_checkpoint('rho-pw copy done')
                end if
                call cpu_time(t_rho1)
              ...
              call density_checkpoint('rho-materialize start')
              call cpu_time(t_rho0)
...
!$omp end parallel
!$omp parallel do reduction(+:send_pre_block_raw) &
!$omp& private(idx_remote, igrid, owner_rank, slot, rho_contrib, spin_offset, ixg, iyg, izg) schedule(static)
                  do idx_remote = 1, valid_remote_grid_count
...
!$omp end parallel do
                call density_checkpoint('rho-materialize done')
                if (dg_frag%is_frag_root) then
    ...
    call density_checkpoint('project-loop done')
    ...
    call density_checkpoint('project-work dealloc start')
    ...
    call density_checkpoint('project-work dealloc done')
    if (enable_density_reconstruct_trace .and. dg_frag%is_frag_root) then
    ...
    call density_checkpoint('comm-section start')
    ...
    if (.not. density_exchange_active) then
      ...
      if (enable_density_reconstruct_trace) then
        write(*,'(1x,a,i0,a,i0,a)') "        density root-exchange: rank=", dg_frag%id, &
          " id_frag=", dg_frag%id_frag, " skipped on non-root orbital rank"
        flush(6)
      end if
    else
    call density_checkpoint('comm-count alloc start')
    ...
    call density_checkpoint('comm-count alloc done')
    charge_owner_local_pre_comm = 0.0d0
    charge_owner_global_pre_comm = 0.0d0
    if (enable_density_stage_contrib_trace) then
...
!$omp end parallel do
    ...
    call density_checkpoint('comm-pack start')
    ...
    call density_checkpoint('comm-pack done')
    if (enable_density_reconstruct_trace) then
      write(*,'(1x,a,i0,a)') "        density collective: rank=", dg_frag%id, " stage=before-alltoallv"
      flush(6)
    end if
    call density_checkpoint('comm-exchange start')
    call cpu_time(t_setup0)
    ...
    call density_checkpoint('comm-exchange done')
    if (enable_density_reconstruct_trace) then
      write(*,'(1x,a,i0,a)') "        density collective: rank=", dg_frag%id, " stage=after-alltoallv"
      flush(6)
    end if
    call density_checkpoint('comm-unpack start')
    call cpu_time(t_setup0)
    do irank = 0, dg_frag%isize - 1
      npts = dg_frag%density_recv_map(irank)%npts
      if (npts <= 0) cycle
...
!$omp atomic update
    ...
    call assert_real_array3_finite_density('rho_bf-after-ifrag-compare', rho_bf, &
      lbound(rho_bf, 1), lbound(rho_bf, 2), lbound(rho_bf, 3), &
      rho_x_lo, rho_x_hi, rho_y_lo, rho_y_hi, rho_z_lo, rho_z_hi, 0)
    call density_checkpoint('comm-buffer dealloc start')
    ...
    call assert_real_array3_finite_density('rho_bf-after-comm-buffer-dealloc', rho_bf, &
      lbound(rho_bf, 1), lbound(rho_bf, 2), lbound(rho_bf, 3), &
      rho_x_lo, rho_x_hi, rho_y_lo, rho_y_hi, rho_z_lo, rho_z_hi, 0)
    call density_checkpoint('comm-buffer dealloc done')
    end if
    call cpu_time(t_comm1)
    time_comm = time_comm + (t_comm1 - t_comm0)
    call density_checkpoint('comm-section done')
    if (enable_density_reconstruct_trace .and. dg_frag%is_frag_root) then
      write(*,'(1x,a,i0,a,i0,a,1pe12.4)') "        density trace: rank=", dg_frag%id, &
    ...
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a)') "        density trace: stage=before-final-density"
      flush(6)
    end if
    if (density_exchange_active) then
    call density_checkpoint('final-copy start')
...
!$omp end parallel do
    ...
    call density_checkpoint('final-copy done')
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a)') "        density trace: stage=after-rho-copy"
      flush(6)
    end if
    if (enable_density_moment_trace) call trace_density_moments('raw')
    if (enable_density_stage_contrib_trace) then
    charge_rho_bf_core_local_raw = 0.0d0
...
!$omp end parallel do
    ...
    call density_checkpoint('final-charge start')
    ...
    call density_checkpoint('final-charge local-sum done')
    ...
    if (enable_density_stage_contrib_trace) then
      call density_checkpoint('final-charge stage-contrib reduce start')
      ...
      call density_checkpoint('final-charge stage-contrib reduce done')
    else
      charge_imported_local = 0.0d0
      charge_imported_global = 0.0d0
    end if
    ...
    call density_checkpoint('final-charge total reduce skipped')
    ...
    call density_checkpoint('final-charge reduce done')
    if (enable_density_stage_contrib_trace) then
      ...
      ! Keep dg-current-decomp columns stable without paying the diagnostic
      ! grid scan/reductions used to split raw charge by ownership stage.
      charge_owner_global_pre_comm = total_charge
      charge_imported_global = 0.0d0
      charge_contract_residual = 0.0d0
    ...
    if (enable_density_moment_trace) call trace_density_moments('final')
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a,i0,a)') "        density collective: rank=", dg_frag%id, " stage=before-final-charge-sum"
      flush(6)
      write(*,'(1x,a,i0,a,1pe12.4)') "        density collective: rank=", dg_frag%id, &
        " stage=after-final-charge-sum elec_num=", dg_frag%elec_num_scaled
      flush(6)
    end if
    call cpu_time(t_norm1)
    time_norm = time_norm + (t_norm1 - t_norm0)
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a,a,1pe12.4)') "        density trace: stage=after-final-density dt=", "", time_norm
      flush(6)
    end if
    end if

    call density_checkpoint('final-dealloc start')
    ...
    if ((enable_density_timing .or. enable_density_reconstruct_trace) .and. dg_frag%id == 0) then
      write(*,'(1x,a,i0,13(a,1pe12.4))') &
        "        density timing: itt=", itt_tag, &
        " total=", t_total1 - t_total0, " cache=", time_cache, &
        " project=", time_project, " comm=", time_comm, " norm=", time_norm, &
        " grid=", time_project_grid_prep, " phi=", time_project_phi_pack, &
        " psi=", time_project_psi, " rho=", time_project_rho, &
        " rho_red=", time_project_rho_reduce, " pack=", time_comm_pack, &
        " exch=", time_comm_exchange, " unpack=", time_comm_unpack
      write(*,'(1x,a,8(a,i0))') "        density parallel:", &
        " pfrag=", dg_frag%isize_frag, " id_frag=", dg_frag%id_frag, &
        " ifrag_local=", ifrag_count, " nbf_max=", nbf_max, &
        " nbf_local_cap=", nbf_frag_cap, " nocc=", nocc_cache, &
        " grid_block=", grid_block_size, " nblocks_max=", nblocks_max
      write(*,'(1x,a,l1,a,l1,4(a,i0))') "        density exchange peers:", &
        " active=", density_exchange_active, " root_only=", density_on_frag_root, &
        " send_peers=", handler_send_peers, " recv_peers=", handler_recv_peers, &
        " send_total=", send_total_count, " recv_total=", recv_total_count
    end if
    if ((enable_density_timing .or. enable_density_reconstruct_trace) .and. dg_frag%id == 0) then
    ...
    if (enable_density_reconstruct_trace .and. dg_frag%id == 0) then
      write(*,'(1x,a,4(a,1pe12.4))') "        density timing detail: project", &
        " setup=", time_project_setup, " phi_blk_build=", time_project_phi_block_build, &
        " over=", time_project_overhead, " dmat=", time_project_dmat_build
      flush(6)
    end if
    call density_checkpoint('final-dealloc done')

  contains

    subroutine density_checkpoint(label)
      implicit none
    ...
    subroutine trace_density_moments(stage_label)
      implicit none
      character(*), intent(in) :: stage_label
      integer :: ix_m, iy_m, iz_m
      real(8) :: rho_val_m, qcell_m, x_m, y_m, z_m
      real(8) :: q_local_m, mx_local_m, my_local_m, mz_local_m
      real(8) :: x2_local_m, y2_local_m, z2_local_m
      real(8) :: rho2_local_m, rhomax_local_m, rhomax_global_m
      real(8) :: moment_local_m(8), moment_global_m(8)
      real(8) :: rhomax_local_arr_m(1), rhomax_global_arr_m(1)
      ...
      z2_local_m = 0.0d0
      rho2_local_m = 0.0d0
      rhomax_local_m = 0.0d0

      ! Moment diagnostics use global grid indices as coordinates.  They are
      ! cheap invariants for checking whether orbital subgroup size changes the
      ! reconstructed density before the Hartree/XC path sees it.
!$omp parallel do collapse(2) reduction(+:q_local_m,mx_local_m,my_local_m,mz_local_m) &
!$omp& reduction(+:x2_local_m,y2_local_m,z2_local_m,rho2_local_m) &
!$omp& reduction(max:rhomax_local_m) private(ix_m,iy_m,iz_m,rho_val_m,qcell_m,x_m,y_m,z_m) schedule(static)
      do iz_m = rho_z_lo, rho_z_hi
        do iy_m = rho_y_lo, rho_y_hi
          do ix_m = rho_x_lo, rho_x_hi
            rho_val_m = rho%f(ix_m, iy_m, iz_m)
            qcell_m = rho_val_m * system%hvol
            x_m = dble(ix_m)
            y_m = dble(iy_m)
            z_m = dble(iz_m)
            q_local_m = q_local_m + qcell_m
            mx_local_m = mx_local_m + qcell_m * x_m
            my_local_m = my_local_m + qcell_m * y_m
            mz_local_m = mz_local_m + qcell_m * z_m
            x2_local_m = x2_local_m + qcell_m * x_m * x_m
            y2_local_m = y2_local_m + qcell_m * y_m * y_m
            z2_local_m = z2_local_m + qcell_m * z_m * z_m
            rho2_local_m = rho2_local_m + rho_val_m * rho_val_m * system%hvol
            rhomax_local_m = max(rhomax_local_m, abs(rho_val_m))
          end do
        end do
      end do
!$omp end parallel do

      moment_local_m = (/q_local_m, mx_local_m, my_local_m, mz_local_m, &
                         x2_local_m, y2_local_m, z2_local_m, rho2_local_m/)
      rhomax_local_arr_m(1) = rhomax_local_m
      call comm_summation(moment_local_m, moment_global_m, 8, dg_frag%icomm)
      call comm_get_max(rhomax_local_arr_m, rhomax_global_arr_m, 1, dg_frag%icomm)
      rhomax_global_m = rhomax_global_arr_m(1)
      if (dg_frag%id == 0) then
        write(*,'(1x,a,i0,a,i0,a,a,9(a,1pe14.6))') &
          '        density moment: itt=', itt_tag, ' pfrag=', dg_frag%isize_frag, &
          ' stage=', trim(stage_label), &
          ' q=', moment_global_m(1), ' mx=', moment_global_m(2), &
          ' my=', moment_global_m(3), ' mz=', moment_global_m(4), &
          ' x2=', moment_global_m(5), ' y2=', moment_global_m(6), &
          ' z2=', moment_global_m(7), ' rho2=', moment_global_m(8), &
          ' rhomax=', rhomax_global_m
        flush(6)
      end if
    end subroutine trace_density_moments
