...
module rt_dg_hybrid_point_density
  ...
  use rt_dg_hybrid_sparse_exchange,only:s_rt_dg_sparse_exchange,accumulate_rt_dg_sparse_density_tiled
#ifdef USE_MPI
  use mpi
#endif
  implicit none
...
contains
  subroutine reconstruct_rt_dg_point_csr_density(comm,plan,point_offsets,support_slots,support_values,&
...
#ifdef USE_MPI
    integer::nocc,ierr,local_bad,global_bad
    ...
    if(ierr/=MPI_SUCCESS)then;message='point-CSR Hybrid validation reduction failed';return;endif
    if(global_bad/=0)then;message='invalid point-CSR Hybrid density contract';return;endif
    call accumulate_rt_dg_sparse_density_tiled(comm,plan,point_offsets,support_slots,support_values,&
      coefficients_owned,occupations,density,min(16,nocc),workspace_peak_bytes,&
    ...
    call MPI_Allreduce(local_bad,global_bad,1,MPI_INTEGER,MPI_MAX,comm,ierr)
    if(ierr/=MPI_SUCCESS)then;ok=.false.;message='point-CSR Hybrid finite-check reduction failed';return;endif
    if(global_bad/=0)then;ok=.false.;message='nonfinite point-CSR Hybrid density';return;endif
    ok=.true.;message=''
#else
