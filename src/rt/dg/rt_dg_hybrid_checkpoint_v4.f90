...
contains
  ...
  subroutine read_rt_dg_hybrid_checkpoint_v4(comm,prefix,payload,ok,message)
    ...
    integer(int64)::transaction_id,basis_fingerprint,operator_fingerprint,operator_structure_fingerprint,&
      scope_fingerprint,payload_fingerprint,stored_digest,actual_digest,actual_size
    integer(int64),allocatable::shard_sizes(:),shard_digests(:)
    integer,allocatable::fragment_ids(:)
    character(32)::magic
    ...
    character(256)::iomsg
    call MPI_Comm_rank(comm,rank,ierr);call MPI_Comm_size(comm,nproc,ierr)
    ok=.false.;message='';payload=s_rt_dg_hybrid_v4_shard();manifest=trim(prefix)//'.manifest';ios=0
    allocate(shard_sizes(nproc),shard_digests(nproc),fragment_ids(nproc))
    if(rank==0)then
    ...
    call shard_name(prefix,transaction_id,rank,shard);ios=0
    open(newunit=unit,file=trim(shard),status='old',access='stream',form='unformatted',&
      action='read',iostat=ios,iomsg=iomsg)
    if(ios==0)then
      inquire(unit=unit,size=actual_size)
      read(unit,iostat=ios,iomsg=iomsg)magic,version,shard_rank,shard_nproc,fragment_id,global_count,&
        global_grid_count,nocc,certified_rank,transaction_id,basis_fingerprint,operator_fingerprint,&
        operator_structure_fingerprint,scope_fingerprint,payload_fingerprint,stored_digest,nrow,&
    ...
    if(ios==0)then
      if(magic/=shard_magic.or.version/=schema_version.or.shard_rank/=rank.or.shard_nproc/=nproc.or.&
        ...
        stored_digest/=shard_digests(rank+1))ios=1
    endif
    if(ios==0)then
    ...
    call MPI_Allreduce(local_bad,global_bad,1,MPI_INTEGER,MPI_MAX,comm,ierr)
    if(ierr/=MPI_SUCCESS.or.global_bad/=0)then
      message='distributed-v4 rank shard is partial, stale, or corrupt';return
    endif
  ...
  subroutine validate_common(comm,payload,bad)
    ...
    call MPI_Allreduce(payload%global_count,imax,1,MPI_INTEGER,MPI_MAX,comm,ierr);if(imin/=imax)bad=1
    call MPI_Allreduce(payload%nocc,imin,1,MPI_INTEGER,MPI_MIN,comm,ierr)
    call MPI_Allreduce(payload%nocc,imax,1,MPI_INTEGER,MPI_MAX,comm,ierr);if(imin/=imax)bad=1
    call MPI_Allreduce(payload%basis_fingerprint,lmin,1,MPI_INTEGER8,MPI_MIN,comm,ierr)
    call MPI_Allreduce(payload%basis_fingerprint,lmax,1,MPI_INTEGER8,MPI_MAX,comm,ierr);if(lmin/=lmax)bad=1
    call MPI_Allreduce(payload%operator_fingerprint,lmin,1,MPI_INTEGER8,MPI_MIN,comm,ierr)
    call MPI_Allreduce(payload%operator_fingerprint,lmax,1,MPI_INTEGER8,MPI_MAX,comm,ierr);if(lmin/=lmax)bad=1
