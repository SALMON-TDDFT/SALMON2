...
contains
  subroutine build_rt_dg_hybrid_structural_graph(comm,global_count,row_ids,basis_values,metric_rows,kinetic_rows,&
    ...
    do i=1,nowned;owners(int(row_ids(i)))=rank+1;owner_marks(int(row_ids(i)))=1;enddo
  ...
  subroutine insert_key(set,key)
    ...
    if(key<=0_int64)error stop 'invalid structural support key'
    ...
    if(set%count*10_int64>=int(capacity,int64)*7_int64)then
      if(capacity>huge(capacity)/2)error stop 'structural support hash capacity overflow'
      call rehash_set(set,2*capacity);capacity=size(set%slot)
    endif
  ...
  subroutine extract_sorted(set,keys)
    ...
    integer::i,q
    if(set%count>int(huge(0),int64))error stop 'structural support count exceeds MPI integer extent'
