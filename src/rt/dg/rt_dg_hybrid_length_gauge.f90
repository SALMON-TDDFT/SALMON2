...
contains
  subroutine propagate_rt_dg_hybrid_length_gauge(comm,metric,operators,coefficients_owned,electric_field,&
    ...
    complex(real64),allocatable::term(:),rhs(:,:),solved(:,:),global_vector(:),applied(:),metric_applied(:)
    ...
    if(int(nowned,int64)>huge(base_bytes)/80_int64.or.int(n,int64)>huge(base_bytes)/16_int64)then
      local_bad=1
    else
      base_bytes=80_int64*int(nowned,int64)
      if(16_int64*int(n,int64)>huge(base_bytes)-base_bytes)local_bad=1
    endif
    call MPI_Allreduce(local_bad,global_bad,1,MPI_INTEGER,MPI_MAX,comm,ierr)
    if(ierr/=MPI_SUCCESS.or.global_bad/=0)then;message='length-gauge workspace receipt overflow';return;endif
    base_bytes=80_int64*int(nowned,int64)+16_int64*int(n,int64)
    allocate(next_coefficients_owned(nowned),term(nowned),rhs(nowned,1),global_vector(n),applied(nowned),&
      metric_applied(nowned),stat=allocation_status)
    ...
    do order=1,max_order
      call assemble_global(term,global_vector,ierr)
      if(ierr/=MPI_SUCCESS)then;call cleanup();message='length-gauge coefficient assembly failed';return;endif
      scale=maxval(abs(global_vector))
      if(scale>safe_scale)then;call cleanup();message='unsafe length-gauge exponential term magnitude';return;endif
      call apply_operator(global_vector,electric_field,applied)
      rhs(:,1)=applied
      call solve_rt_dg_hybrid_metric(comm,metric,rhs,tolerance,inner_cap,solved,inner_iterations,inner_residual,&
        solver_bytes,solver_fingerprint,solver_ok,solver_message)
    ...
    call assemble_global(next_coefficients_owned,global_vector,ierr)
    if(ierr/=MPI_SUCCESS)then;call cleanup();message='length-gauge observable assembly failed';return;endif
    scale=maxval(abs(global_vector))
    if(scale>safe_scale)then;call cleanup();message='unsafe propagated observable magnitude';return;endif
    call apply_metric(global_vector,metric_applied)
    ...
    call apply_operator(global_vector,[0d0,0d0,0d0],applied)
    ...
    do i=1,3
      call apply_position(global_vector,i,applied)
      local_dot=sum(conjg(next_coefficients_owned)*applied)
      call MPI_Allreduce(local_dot,global_dot,1,MPI_DOUBLE_COMPLEX,MPI_SUM,comm,ierr)
      if(ierr/=MPI_SUCCESS.or.abs(aimag(global_dot))>100d0*tolerance*max(1d0,abs(real(global_dot))))then
    ...
    enddo
    scale=maxval(abs(global_vector));quantization_limit=0.25d0*real(huge(0_int64),real64)*100d0*tolerance
    if(scale>quantization_limit)then;call cleanup();message='length-gauge fingerprint range is unsafe';return;endif
    do i=1,n
      quantized=nint(real(global_vector(i))/(100d0*tolerance),int64);fingerprint=ieor(ishftc(fingerprint,9),quantized)
      quantized=nint(aimag(global_vector(i))/(100d0*tolerance),int64);fingerprint=ieor(ishftc(fingerprint,9),quantized)
    enddo
  ...
  contains
    ...
    subroutine assemble_global(local_values,global_values,status)
      ...
      do k=1,nowned;global_values(int(metric%owned_row_ids(k)))=local_values(k);enddo
      call MPI_Allreduce(MPI_IN_PLACE,global_values,n,MPI_DOUBLE_COMPLEX,MPI_SUM,comm,status)
    end subroutine assemble_global
    subroutine apply_metric(global_values,local_values)
      ...
      do k=1,nowned
        do row=metric%row_offsets(k),metric%row_offsets(k+1)-1
          local_values(k)=local_values(k)+metric%values(row)*global_values(metric%column_ids(row))
        enddo
      enddo
    end subroutine apply_metric
    subroutine apply_operator(global_values,field,local_values)
      complex(real64),intent(in)::global_values(:);real(real64),intent(in)::field(3)
      ...
      do k=1,nowned
        do row=operators%row_offsets(k),operators%row_offsets(k+1)-1
          local_values(k)=local_values(k)+(operators%hamiltonian_values(row)+&
            sum(field*operators%position_values(:,row)))*global_values(operators%column_ids(row))
        enddo
      enddo
    end subroutine apply_operator
    subroutine apply_position(global_values,component,local_values)
      complex(real64),intent(in)::global_values(:);integer,intent(in)::component
      complex(real64),intent(out)::local_values(:)
      local_values=(0d0,0d0)
      do k=1,nowned
        do row=operators%row_offsets(k),operators%row_offsets(k+1)-1
          local_values(k)=local_values(k)+operators%position_values(component,row)*global_values(operators%column_ids(row))
        enddo
      enddo
    ...
    subroutine cleanup()
      ...
      if(allocated(global_vector))deallocate(global_vector)
