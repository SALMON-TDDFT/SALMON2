!=======================================================================
  ...
  subroutine extract_basis_from_orbitals(dg_frag, psi, i_local)
    ...
    ! SOI path keeps complex basis in phi_frag_c; phi_frag stores its real part for compatibility.
    do ispin = 1, dg_frag%nspin
      nstate_loc = min(nb_local, nstate_use)
      do istate = 1, nstate_loc
        io_idx = io_lb + istate - 1
        ...
        do jstate = 1, istate - 1
          ...
          do lz = 1, ndom(3)
            do ly = 1, ndom(2)
              do lx = 1, ndom(1)
                if (allocated(dg_frag%phi_frag_c)) then
                  proj = proj + conjg(dg_frag%phi_frag_c(lx, ly, lz, jstate, i_local)) * &
                                dg_frag%phi_frag_c(lx, ly, lz, istate, i_local) * hvol
                else
                  proj = proj + cmplx(dg_frag%phi_frag(lx, ly, lz, istate, i_local), 0.0d0, kind=8) * &
                                cmplx(dg_frag%phi_frag(lx, ly, lz, jstate, i_local), 0.0d0, kind=8) * hvol
                end if
              end do
            end do
          end do
              ...
              do lx = 1, ndom(1)
                if (allocated(dg_frag%phi_frag_c)) then
                  dg_frag%phi_frag_c(lx, ly, lz, istate, i_local) = &
                    dg_frag%phi_frag_c(lx, ly, lz, istate, i_local) - &
                    proj * dg_frag%phi_frag_c(lx, ly, lz, jstate, i_local)
                else
                  dg_frag%phi_frag(lx, ly, lz, istate, i_local) = &
                    dg_frag%phi_frag(lx, ly, lz, istate, i_local) - &
                    real(proj, kind=8) * dg_frag%phi_frag(lx, ly, lz, jstate, i_local)
                end if
              end do
            end do
          end do
        ...
        do lz = 1, ndom(3)
          do ly = 1, ndom(2)
            do lx = 1, ndom(1)
              if (allocated(dg_frag%phi_frag_c)) then
                normv = normv + abs(dg_frag%phi_frag_c(lx, ly, lz, istate, i_local))**2
              else
        ...
        if (allocated(dg_frag%phi_frag_c)) then
          dg_frag%phi_frag_c(1:ndom(1), 1:ndom(2), 1:ndom(3), istate, i_local) = &
            dg_frag%phi_frag_c(1:ndom(1), 1:ndom(2), 1:ndom(3), istate, i_local) / normv
          dg_frag%phi_frag(1:ndom(1), 1:ndom(2), 1:ndom(3), istate, i_local) = &
            real(dg_frag%phi_frag_c(1:ndom(1), 1:ndom(2), 1:ndom(3), istate, i_local), kind=8)
        else
          dg_frag%phi_frag(1:ndom(1), 1:ndom(2), 1:ndom(3), istate, i_local) = &
