...
C
      SUBROUTINE PZFFT3DV_MOD(A,B,NX,NY,NZ,NPUY,NPUZ,IOPT,ICOMMY,ICOMMZ)
...
C     1                  MPI_DOUBLE_COMPLEX,ICOMMY,IERR)
      CALL CONVERT_1D(BYZX2,AYZXP,NN,ICOMMY,NPUY)
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO
      DO 70 K=1,NNZZ*NNXY
...
C     1                  NN/NPUZ,MPI_DOUBLE_COMPLEX,ICOMMZ,IERR)
      CALL CONVERT_1D(AZXY2,BZXYP,NN,ICOMMZ,NPUZ)
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO
...
C     1                  MPI_DOUBLE_COMPLEX,ICOMMZ,IERR)
      call CONVERT_1D(B,AXYZP,NN,ICOMMZ,NPUZ)
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO
      DO 220 L=1,NPUZ
...
C     1                    MPI_DOUBLE_COMPLEX,ICOMMZ,IERR)
      call CONVERT_1D(BXYZP,A,NN,ICOMMZ,NPUZ)
!$OMP END MASTER
!$OMP BARRIER
      END IF
C
...
C     1                  MPI_DOUBLE_COMPLEX,ICOMMZ,IERR)
      CALL CONVERT_1D(AZXYP,BZXY2,NN,ICOMMZ,NPUZ)
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO
...
C     1                  MPI_DOUBLE_COMPLEX,ICOMMY,IERR)
      CALL CONVERT_1D(BYZXP,AYZX2,NN,ICOMMY,NPUY)
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO
