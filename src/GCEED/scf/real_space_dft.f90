...
type(s_scalar) :: sVh
type(s_scalar),allocatable :: V_local(:),srho(:,:),srho_s(:,:),sVxc(:)
...
!------------------------------ New calculation
  ...
  case default
    ...
    allocate(srho(1,1))
    allocate(V_local(system%nspin),sVxc(system%nspin))
...
DFT_Iteration : do iter=1,iDiter(img)
  ...
  if(iscf_order==1)then
    ...
    call scf_iteration(mg,system,info,stencil,srg_ob_1,spsi,srho,srho_s,iflag,itotmst,mst,ilsda,nproc_ob, &
