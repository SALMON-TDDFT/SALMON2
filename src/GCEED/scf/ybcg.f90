...
orbital : do iob=iobsta(is),iobend(is)
  ...
  call check_corrkob_ud(iob,1,icorr,ilsda,nproc_ob,iparaway_ob,itotmst,k_sta,k_end,nproc_ob_spin,mst)
  elp2(2)=get_wtime()
...
!$OMP parallel do private(iz,iy,ix) 
    ...
    do job=iobsta(is),iob-1
      ...
      call check_corrkob_ud(job,1,jcorr,ilsda,nproc_ob,iparaway_ob,itotmst,k_sta,k_end,nproc_ob_spin,mst)
      if(jcorr==1)then
