...
!$OMP parallel do collapse(3) private(is,iz,iy,ix)
  ...
  case(3)
    if(use_singlescale=='n') then
      system%vec_Ac(1:3) = rt%Ac_ext(1:3,itt) + A_ind(1:3,itt)
      system%vec_E(1:3) = -((rt%Ac_ext(1:3,itt) + A_ind(1:3,itt))-(rt%Ac_ext(1:3,itt-1) + A_ind(1:3,itt-1)))/dt
  ...
  !$OMP parallel do collapse(3) private(is,iz,iy,ix)
    ...
    case(3)
      if(use_singlescale=='y') then
        ...
        system%vec_Ac(1:3) = rt%Ac_ext(1:3,itt+1) + A_ind(1:3,itt+1)
        system%vec_E(1:3) = -((rt%Ac_ext(1:3,itt+1) + A_ind(1:3,itt+1))-(rt%Ac_ext(1:3,itt) + A_ind(1:3,itt)))/dt
