...
contains
  ...
  subroutine check_condition_jm
    ...
    call condition_yn_jm(yn_out_pdos,'yn_out_tm',  'n')
  ...
  subroutine make_rho_jm(lg,mg,system,info,rho_jm)
    ...
    !allocate density, radius, and id-matrix
    allocate(dens(num_jm),radi(num_jm)); dens(:)=0.0d0; radi(:)=0.0d0;
    ...
    if (trim(shape_file_jm)=='none')then
      ...
      !**************************************************************************************!
      stop "*** shape_file_jm still cannot be used ***"
    end if
    
    !check charge neutrality
    call check_neutral_jm(charge_sum,charge_error)
    ...
    if(comm_is_root(nproc_id_global))then
      ...
      if(trim(shape_file_jm)=='none')then
        ...
        write(*,'("  Positive background charge density is generated by cube file.")')
      end if
      if(sum(mod_rs_bohr_jm(:))==0.0d0) then
        write(*,'("  Wigner-Seitz is set as follows:")')
        do ii=1,num_jm
          write(*, '(A,I3,A,E23.15E3)') '    rs_bohr_jm(',ii,') =', rs_bohr_jm(ii)
        end do
      else
        write(*,'("  To keep charge neutrality, Wigner-Seitz is modified as follows:")')       
        do ii=1,num_jm
   ...
   contains
    ...
    !+ check charge neutrality +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    subroutine check_neutral_jm(sum_c,err)
      implicit none
...
!$omp end parallel
      ...
      err   = abs( (sum_c - dble(sum(nelec_jm(:)))) / dble(sum(nelec_jm(:))) )
