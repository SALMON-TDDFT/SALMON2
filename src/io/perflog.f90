...
!    use flops
  ...
  subroutine write_performance(fd,mode)
    ...
    call set( 3, LOG_CHEFSI_FILTER,        'Chebyshev filter')
    call set( 4, LOG_CHEFSI_ORTHO,         'orthonormalization')
    call set( 5, LOG_CHEFSI_RAYLEIGH_RITZ, 'Rayleigh-Ritz')
    call set( 6, LOG_CHEFSI_RESIDUAL,      'residual')
    call set( 7, LOG_CHEFSI_EXPORT,        'export coefficients')
    call set( 8, LOG_CHEFSI_H_APPLY,       'H apply total')
    call set( 9, LOG_CHEFSI_H_DIAG,        '::diagonal block')
    call set(10, LOG_CHEFSI_H_COMM_POST,   '::post halo comm.')
    call set(11, LOG_CHEFSI_H_RECV_WAIT,   '::wait halo receive')
    call set(12, LOG_CHEFSI_H_HALO,        '::off-diagonal blocks')
    call set(13, LOG_CHEFSI_H_SEND_WAIT,   '::wait halo send')
    call set(14, LOG_CHEFSI_REDISTRIBUTE,  'data redistribution')
    call set(15, LOG_CHEFSI_PROJECT,       'projected matrix')
    call set(16, LOG_CHEFSI_PROJECT_EIGEN, 'projected eigensolve')
    call set(17, LOG_CHEFSI_ROTATE,        'Ritz rotation')
    call write_loadbalance(fd, 17, tsrc, headers, mode)
