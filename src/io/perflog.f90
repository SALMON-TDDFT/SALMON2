...
contains
  ...
  subroutine write_performance(fd,mode)
    ...
    call set(2, LOG_TE_ISOLATED_COMM_COLL,  'isolated comm. coll')
    call set(3, LOG_TE_PERIODIC_CALC,       'periodic calc.')
    call set(4, LOG_TE_PERIODIC_COMM_COLL,  'periodic comm. coll.')
    call set(5, LOG_EIGEN_ENERGY_CALC,      'eigen_energy calc.')
    call set(6, LOG_EIGEN_ENERGY_COMM_COLL, 'eigen_energy comm. coll.')
    call set(7, LOG_EIGEN_ENERGY_HPSI,      'eigen_energy hpsi')
    call write_loadbalance(fd, 7, tsrc, headers, mode)

    call set(0, 0, 'density_matrix module')
    call set( 1, LOG_DENSITY_MATRIX_CALC,          'density_matrix calc.')
    call set( 2, LOG_DENSITY_MATRIX_COMM_COLL,     'density_matrix comm. coll.')
    call set( 3, LOG_DENSITY_MATRIX_COMM_HALO,     'density_matrix comm. halo')
    call set( 4, LOG_DENSITY_CALC,                 'density calc.')
    call set( 5, LOG_DENSITY_COMM_COLL,            'density comm. coll.')
    call set( 6, LOG_CURRENT_CALC,                 'current calc.')
    call set( 7, LOG_CURRENT_CALC_UVPSI_RDIVIDED,  'current rDivided uVpsi calc.')
    call set( 8, LOG_CURRENT_COMM_COLL,            'current comm. coll.')
    call set( 9, LOG_CURRENT_COMM_HALO,            'current comm. halo.')
    call set(10, LOG_MCURRENT_CALC,                'micro-current calc.')
    call set(11, LOG_MCURRENT_COMM_COLL,           'micro-current comm. coll.')
    call write_loadbalance(fd, 11, tsrc, headers, mode)

    call set(0, 0, 'conjugate_gradient module')
    call set(1, LOG_GSCG_ISOLATED_CALC,      'isolated calc.')
    call set(2, LOG_GSCG_ISOLATED_COMM_COLL, 'isolated comm. coll.')
    call set(3, LOG_GSCG_ISOLATED_HPSI,      'isolated hpsi')
    call set(4, LOG_GSCG_PERIODIC_CALC,      'periodic calc.')
    call set(5, LOG_GSCG_PERIODIC_COMM_COLL, 'periodic comm. coll.')
    call set(6, LOG_GSCG_PERIODIC_HPSI,      'periodic hpsi')
    call write_loadbalance(fd, 6, tsrc, headers, mode)

    call set(0, 0, 'subspace_diag module')
    call set(1, LOG_SSDG_ISOLATED_CALC,      'isolated calc.')
    call set(2, LOG_SSDG_ISOLATED_COMM_COLL, 'isolated comm. coll.')
    call set(3, LOG_SSDG_ISOLATED_HPSI,      'isolated hpsi')
    ...
    call set(2, LOG_SSDG_SO_ISOLATED_COMM_COLL, 'isolated comm. coll.')
    call set(3, LOG_SSDG_SO_ISOLATED_HPSI,      'isolated hpsi')
    call set(4, LOG_SSDG_SO_PERIODIC_CALC,      'periodic calc.')
    call set(5, LOG_SSDG_SO_PERIODIC_COMM_COLL, 'periodic comm. coll.')
    ...
    call set(3, LOG_UHPSI_STENCIL,        'stencil')
    call set(4, LOG_UHPSI_SUBTRACTION,    'subtraction')
    call set(5, LOG_UHPSI_PSEUDO,         'pseudo-pt')
