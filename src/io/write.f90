  ...
  subroutine write_rt_xc(it,ofl,dt,rt)
    ...
    if (it<0) then
      ...
      if(tdcdft_screening=='elf') then
        write(ofl%fh_rt_xc,'(a)') '# extra columns: alpha_used Q_ELF P_xyz Q_initial; instantaneous held ELF measure'
      else if(tdcdft_screening/='none') then
        write(ofl%fh_rt_xc,'(a)') &
      ...
      if(tdcdft_screening=='elf') then
        write(ofl%fh_rt_xc,'(13(1x,es24.16e3))') it*dt*t_unit_time%conv, &
          rt%Ac_xc(:,it)*t_unit_ac%conv,exc*t_unit_elec%conv,rt%xc_alpha,rt%xc_response, &
          rt%xc_polarization,rt%xc_elf_reference
      else if(tdcdft_screening/='none') then
