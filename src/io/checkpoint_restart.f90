        ...
        case (read_mode)
          if (allocated(spsi%rwf)) then
            ...
            if (source_type == MPI_DOUBLE) then
              ...
              MPI_CHECK(MPI_Type_create_subarray(7, gsize, lsize, lstart, MPI_ORDER_FORTRAN, source_type, local_type_d, ierr))
              MPI_CHECK(MPI_Type_commit(local_type_d, ierr))
              MPI_CHECK(MPI_File_set_view(mfile, 0_MPI_OFFSET_KIND, local_type_d, global_type, 'native', MPI_INFO_NULL, ierr))
