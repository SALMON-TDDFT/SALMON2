...
subroutine write_singlescale(odir,lg,ng,info,singlescale,Ac,div_Ac,is_self_checkpoint)
  ...
  if (is_self_checkpoint) then
    ...
    if(yn_gbp=='y') then
      ...
      write(iu1_w) singlescale%g_old(1:lg%num(1),1:ng%num(2),1:ng%num(3),0:3)
...
!$omp parallel do collapse(2) private(iz,iy,ix)
  ...
  !$omp parallel do collapse(2) private(iz,iy,ix)
      ...
      do iy=ng%is(2),ng%ie(2)
      do ix=ng%is(1),ng%ie(1)
        z0(ix,iy,iz,0:3,1) = singlescale%f_old(ix,iy,iz,0:3)
        z0(ix,iy,iz,0:3,2) = singlescale%g_old(ix,iy,iz,0:3)
      end do
      end do
      end do
...
subroutine restart_singlescale(comm,lg,ng,singlescale,Ac,div_Ac)
  ...
  if (iself) then
    ...
    if(yn_gbp=='y') then
      ...
      read(iu1_r) singlescale%g_old(1:lg%num(1),1:ng%num(2),1:ng%num(3),0:3)
...
!$omp parallel do collapse(2)
    ...
    if(yn_gbp=='y') then
      ...
      do iy=ng%is(2),ng%ie(2)
      do ix=1,lg%num(1)
        singlescale%f_old(ix,iy,iz,0:3) = zbox(ix,iy,iz,0:3,1)
        singlescale%g_old(ix,iy,iz,0:3) = zbox(ix,iy,iz,0:3,2)
      end do
      end do
      end do
    end if
