...
contains
  ...
  complex(8) :: zpol(3), zphase, zAc(3), zAc_x_plus(3), zAc_x_minus(3), zAc_y_plus(3), zAc_y_minus(3), div_perp
  ...
  zphase = exp(zi * phase)
  zAc(:) = f0 / omega1 * (cos(pi * tt / tw1))**npower * window * radial_factor * zpol(:) * zphase
  A_ext(1:2) = -aimag(zAc(1:2))

  call calc_optical_vortex_Ac_complex(t, x + 0.5d0 * hgs(1), y, nx, ny, hgs, zAc_x_plus)
  call calc_optical_vortex_Ac_complex(t, x - 0.5d0 * hgs(1), y, nx, ny, hgs, zAc_x_minus)
  call calc_optical_vortex_Ac_complex(t, x, y + 0.5d0 * hgs(2), nx, ny, hgs, zAc_y_plus)
  call calc_optical_vortex_Ac_complex(t, x, y - 0.5d0 * hgs(2), nx, ny, hgs, zAc_y_minus)
  ! Paraxial Coulomb-gauge correction: div_perp Ac_perp + d Ac_z/dz ~= 0.
  div_perp = (zAc_x_plus(1) - zAc_x_minus(1)) / hgs(1) + (zAc_y_plus(2) - zAc_y_minus(2)) / hgs(2)
  A_ext(3) = (cspeed_au / omega1) * real(div_perp)
  ...
  real(8) :: dA_dt(3), dA_dx(3), dA_dy(3), dA_dz(3), tt
  ...
  dA_dz(:) = -dA_dt(:) / cspeed_au

  E_ext(:) = -dA_dt(:)
  B_ext(1) = cspeed_au * (dA_dy(3) - dA_dz(2))
  B_ext(2) = cspeed_au * (dA_dz(1) - dA_dx(3))
  B_ext(3) = cspeed_au * (dA_dx(2) - dA_dy(1))
  end subroutine calc_optical_vortex_EB_ext

  subroutine calc_optical_vortex_Ac_complex(t, x, y, nx, ny, hgs, zAc)
  use math_constants, only: zi, pi
  use salmon_global, only: I_wcm2_1, E_amplitude1, ae_shape1, tw1, omega1, phi_cep1, t1_start, &
    & optical_vortex_center_x, optical_vortex_center_y, optical_vortex_radius, &
    & optical_vortex_charge, optical_vortex_polarization
  ...
  case('Acos8'); npower = 8
  case default
    stop 'unsupported ae_shape1 in calc_optical_vortex_Ac_complex'
  end select
  ...
  case('right_circular')
    ...
    zpol(2) = dcmplx(0d0, 1d0 / sqrt(2d0))
  case default
    stop 'unsupported optical_vortex_polarization in calc_optical_vortex_Ac_complex'
  end select

  phase = omega1 * tt + phi_cep1 * 2d0 * pi + dble(optical_vortex_charge) * phi_xy
  zAc(:) = f0 / omega1 * (cos(pi * tt / tw1))**npower * window * radial_factor * zpol(:) * exp(zi * phase)
  end subroutine calc_optical_vortex_Ac_complex
