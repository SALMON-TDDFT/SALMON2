...
!===================================================================================================================================
  ...
  subroutine calc_virial(system,pp,fg,info,mg,stencil,poisson,srg,ppg,tpsi,ewald,energy,virial)
    ...
    case('y')
       !$omp parallel do collapse(2) private(ix,iy,iz,ia,r,g,G2,Gd,rho_e,egd,rtmp) reduction(+:P_tmp_loc)
       ...
       do ix=mg%is(1),mg%ie(1)
          ...
          rho_e = poisson%zrhoG_ele(ix,iy,iz) !modify here for virial
          ...
          do ia=info%ia_s,info%ia_e
             ...
             Gd = sum(g(:)*r(:))
             egd = exp(zI*Gd)
             rtmp = real( rho_e*ppg%div_GzVG_ion(ix,iy,iz,ia)*egd  )
             if (abs(rtmp)<1d-32) cycle
             P_tmp_loc = P_tmp_loc - rtmp
             !P_tmp_loc = P_tmp_loc - real( rho_e*ppg%div_GzVG_ion(ix,iy,iz,ia)  )
             !P_tmp_loc = P_tmp_loc - real( ppg%div_GzVG_ion(ix,iy,iz,ia)  )
             if ((info%ia_s==2).and.(info%ia_e==2)) then!write(*,*) "debug",ix,iy,iz,rtmp,P_tmp_loc
                if ((ix==15).and.(iy==8).and.(iz==2)) write(*,*) "debug",rho_e,ppg%div_GzVG_ion(ix,iy,iz,ia),egd,rtmp,P_tmp_loc
             end if
             !P_tmp_loc = P_tmp_loc - real( rho_e*egd  )
          end do
       end do
       end do
       end do
