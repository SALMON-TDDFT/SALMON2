...
subroutine hartree(lg,mg,ng,info_field,system,poisson,srg_ng,stencil,srho,sVh,fg)
  ...
  case(3)
    select case(yn_ffte)
    case('n')
      call poisson_periodic(lg,mg,ng,system,info_field,srho%f,sVh%f,fg%zrhoG_ele,fg%zrhoG_ele_tmp,poisson)
    case('y')
      call poisson_ffte(lg,mg,ng,info_field,srho%f,sVh%f,fg%zrhoG_ele,fg%zrhoG_ele_tmp,system%hgs,poisson)
    end select
  end select
