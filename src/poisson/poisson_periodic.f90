...
!$OMP parallel do private(iz,iy,ix)
  ...
  call comm_summation(poisson%trho2z,poisson%trho3z,mg%num(1)*mg%num(2)*lg%num(3),info_field%icomm_ffte(3))
...
!$OMP parallel do private(kz,ky,ix)
  ...
  call comm_summation(poisson%ff1x,poisson%ff2x,lg%num(1)*mg%num(2)*mg%num(3),info_field%icomm_ffte(1))
...
!$OMP parallel do private(kz,ky,kx,n,gx,gy,gz,g2,kkx,kky,kkz)
  ...
  call comm_summation(fg%zrhoG_ele_tmp,fg%zrhoG_ele,lg%num(1)*lg%num(2)*lg%num(3),fg%icomm_G)
