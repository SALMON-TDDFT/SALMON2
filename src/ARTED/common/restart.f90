...
!  BACKUP(dump_filename)
  ...
  BACKUP(unit_energy)
  BACKUP(unit_charge)
  BACKUP(yn_domain_parallel)
  BACKUP(nproc_ob)
  BACKUP(nproc_domain_orbital(1))
  BACKUP(nproc_domain_orbital(2))
  BACKUP(nproc_domain_orbital(3))
  BACKUP(nproc_domain_general(1))
  BACKUP(nproc_domain_general(2))
  BACKUP(nproc_domain_general(3))
...
!  BACKUP(iperiodic)
  ...
  BACKUP(nfsset_every)
  BACKUP(nscf)
  BACKUP(yn_subspace_diagonalization)
  BACKUP(convergence)
  BACKUP(threshold)
  BACKUP(trans_longi)
  BACKUP(ae_shape1)
  ...
  BACKUP(yn_out_elf)
  BACKUP(out_old_dns)
  BACKUP(yn_out_dns_rt)
  BACKUP(out_dns_rt_step)
  BACKUP(yn_out_elf_rt)
  BACKUP(out_elf_rt_step)
  BACKUP(yn_out_estatic_rt)
  BACKUP(out_estatic_rt_step)
  BACKUP(format_voxel_data)
  BACKUP(nsplit_voxel_data)
  BACKUP(layout_multipole)
  BACKUP(num_multipole_xyz(1))
  BACKUP(num_multipole_xyz(2))
  BACKUP(num_multipole_xyz(3))
  BACKUP(newald)
  BACKUP(aewald)
  BACKUP(Kion)
  BACKUP(Rion)
  BACKUP(Rion_red)
  BACKUP(flag_opt_atom)
! inputoutput
