#!/usr/bin/env python3
"""Compare DG current-derived response spectra from *_rt.data files.

The comparison is intentionally lightweight: it reads the matter current
columns from SALMON RT output, applies the same preprocessing to all inputs,
and reports peak and RMS differences relative to a reference trajectory.
"""
...
def summarize(
    ...
    ref_time, ref_current = load_rt_current(ref_path)
    energy, ref_amp = response_spectrum(ref_time, ref_current, axis, damping, pad_factor, skip, skip_time_au, tmax_au, detrend)
    ...
    for path, meta in zip(cmp_paths, cmp_meta, strict=True):
        time, current = load_rt_current(path)
        energy_cmp, amp_cmp = response_spectrum(time, current, axis, damping, pad_factor, skip, skip_time_au, tmax_au, detrend)
