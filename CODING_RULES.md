...
- For internal subprograms in a host subprogram's `contains` section, use host association for entities that are part of the host procedure's operation. Do not duplicate `use` statements or pass host variables as arguments solely to make them available to the internal subprogram. Keep `implicit none` in each internal subprogram. Use a local `use, only:` for dependencies specific to that subprogram, and pass arguments explicitly when doing so makes the dependency clearer or keeps the procedure independent of host state.
