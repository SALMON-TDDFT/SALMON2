...
work=Path('/private/tmp/salmon-si-time-wannier-dense');d=work/case;d.mkdir(parents=True,exist_ok=True)
...
(d/'inputfile').write_text(s);(out/f'{case}_dense.inp').write_text(s)
...
(out/f'{case}_dense_status.json').write_text(json.dumps(status,indent=2)+'\n');print(status,flush=True)
